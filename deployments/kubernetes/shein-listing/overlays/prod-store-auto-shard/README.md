...
- `TASK_PROCESSOR_RABBITMQ_AUTO_SHARD_CANDIDATE_NODES=shein-listing-store-a,shein-listing-store-b`

使用前请至少确认两件事：

1. 两个 Deployment 的镜像都换成包含自动分片代码的新 tag
2. `shein-listing-secret` 里 Redis 与 management API 配置可用
...
1. 先保留 2 个 shard 节点，验证 Redis owner 是否自动生成
