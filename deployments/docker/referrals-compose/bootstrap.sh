...
if [ -f "$marker" ]; then
  for path in \
    "$trusted_ca/root-ca.pem" \
    "$traefik_tls/localhost.crt" \
    "$traefik_tls/localhost.key" \
    "$identity_db_secret/identity-db-password" \
    ...
    "$referral_db_owner_secret/referral-db-password" \
    "$referral_runtime_secret/referral-runtime-password" \
    "$zitadel_api_secrets/zitadel-masterkey" \
    "$zitadel_api_secrets/runtime-config.yaml" \
    "$tofu_inputs/operator-password" \
    "$frontend_secrets/auth-secret"; do
    test -f "$path"
  done
  exit 0
fi
...
mkdir -p \
  ...
  "$traefik_tls" \
  "$identity_db_secret" \
  "$source_db_owner_secret" \
  "$source_runtime_secret" \
  "$commercial_db_owner_secret" \
  "$commercial_runtime_secret" \
  "$referral_db_owner_secret" \
  "$referral_runtime_secret" \
  "$zitadel_api_secrets" \
  "$tofu_inputs" \
  "$frontend_secrets"
...
write_random 32 "$frontend_secrets/auth-secret"
{
  printf 'Local!'
  openssl rand -hex 18
} > "$tofu_inputs/operator-password.tmp"
...
openssl req -x509 -newkey rsa:2048 -nodes -days 365 -sha256 \
  ...
  -keyout "$work/root-ca.key" -out "$trusted_ca/root-ca.pem"
openssl req -newkey rsa:2048 -nodes -sha256 -config "$work/openssl.cnf" \
  -keyout "$traefik_tls/localhost.key" -out "$work/localhost.csr"
openssl x509 -req -days 365 -sha256 -CA "$trusted_ca/root-ca.pem" -CAkey "$work/root-ca.key" \
  -CAcreateserial -in "$work/localhost.csr" -out "$traefik_tls/localhost.crt" \
  -extensions req_extensions -extfile "$work/openssl.cnf"
