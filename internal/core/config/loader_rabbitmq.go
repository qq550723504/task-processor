...
func BuildRabbitMQConfig(v *viper.Viper) *RabbitMQConfig {
	return &RabbitMQConfig{
		...
		Node: NodeConfig{
			...
			OwnedBuckets:    getIntSlice(v, "rabbitmq.node.ownedBuckets"),
		...
		AutoShard: AutoShardConfig{
			Enabled:             v.GetBool("rabbitmq.autoShard.enabled"),
			Role:                v.GetString("rabbitmq.autoShard.role"),
			Platform:            v.GetString("rabbitmq.autoShard.platform"),
			Interval:            getDuration(v, "rabbitmq.autoShard.interval", 30),
			PageSize:            v.GetInt("rabbitmq.autoShard.pageSize"),
			...
			TargetStoresPerNode: v.GetInt("rabbitmq.autoShard.targetStoresPerNode"),
			NodeWeights:         getStringIntMap(v, "rabbitmq.autoShard.nodeWeights"),
		},
		DeadLetter: DeadLetterConfig{
			Enabled:   v.GetBool("rabbitmq.deadLetter.enabled"),
			QueueName: v.GetString("rabbitmq.deadLetter.queueName"),
		},
		ProcessingTimeout: ProcessingTimeoutConfig{
			Enabled:        v.GetBool("rabbitmq.processingTimeoutWatchdog.enabled"),
			Interval:       getDuration(v, "rabbitmq.processingTimeoutWatchdog.intervalSeconds", 300),
			TimeoutMinutes: v.GetInt("rabbitmq.processingTimeoutWatchdog.timeoutMinutes"),
			RecoveryLimit:  v.GetInt("rabbitmq.processingTimeoutWatchdog.recoveryLimit"),
		},
		StaleQueued: StaleQueuedConfig{
			Enabled:        v.GetBool("rabbitmq.staleQueuedWatchdog.enabled"),
			Interval:       getDuration(v, "rabbitmq.staleQueuedWatchdog.intervalSeconds", 300),
			TimeoutMinutes: v.GetInt("rabbitmq.staleQueuedWatchdog.timeoutMinutes"),
			RecoveryLimit:  v.GetInt("rabbitmq.staleQueuedWatchdog.recoveryLimit"),
		},
