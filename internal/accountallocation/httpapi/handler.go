...
func (h *Handler) list(c *gin.Context) {
	...
	response := snapshotResponse{SchemaVersion: "account-member-token-allocation-v1", OrganizationID: snapshot.OrganizationID, Metric: snapshot.Metric, WindowStart: snapshot.WindowStart.UTC().Format(time.RFC3339Nano), WindowEnd: snapshot.WindowEnd.UTC().Format(time.RFC3339Nano), Enterprise: enterpriseViewResponse(snapshot.Enterprise), Members: make([]memberResponse, 0, len(page.Items))}
	for _, member := range page.Items {
		if member.State != "active" {
			continue
		}
...
type snapshotResponse struct {
	...
	WindowStart    string             `json:"windowStart"`
	WindowEnd      string             `json:"windowEnd"`
	Enterprise     enterpriseResponse `json:"enterprise"`
