...
func runProductEnrichTenantScopeContract(t *testing.T, newRepository func(*testing.T) productenrich.TaskRepository) {
	...
	t.Run("CreateTask", func(t *testing.T) {
		t.Run("matching scoped envelope", func(t *testing.T) {
			...
			task := &productenrich.Task{ID: "create-own", Status: productenrich.TaskStatusPending, PersistedExecutionEnvelope: productEnrichExecutionEnvelope(" tenant-a ")}
		...
		for _, tc := range []struct {
			...
			{name: "cross tenant", envelope: productEnrichExecutionEnvelope("tenant-b"), legacyID: "tenant-a"},
			...
			t.Run(tc.name, func(t *testing.T) {
				repo := newRepository(t)
				task := &productenrich.Task{ID: "create-rejected", TenantID: tc.legacyID, Status: productenrich.TaskStatusPending, PersistedExecutionEnvelope: tc.envelope}
				require.ErrorIs(t, repo.CreateTask(tenantA, task), productenrich.ErrTaskNotFound)
				_, err := repo.GetTask(unscoped, task.ID)
				require.ErrorIs(t, err, productenrich.ErrTaskNotFound, "rejected create persisted a row")
			})
		}
	})
	...
	t.Run("legacy owner does not authorize an absent execution envelope", func(t *testing.T) {
		repo := newRepository(t)
		require.NoError(t, repo.CreateTask(unscoped, &productenrich.Task{
			ID: "legacy-task", Status: productenrich.TaskStatusPending, TenantID: "tenant-a", UserID: "legacy-user-a",
		}))
