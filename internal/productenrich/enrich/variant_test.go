...
func newTestVariantGenerator(t *testing.T, llmResp string, llmErr error) productenrich.VariantGenerator {
	t.Helper()
...
func TestGenerateSpecs_NilAnalysis_ReturnsError(t *testing.T) {
	v := newTestVariantGenerator(t, "{}", nil)
...
func TestGenerateSpecs_ValidJSON_ParsesSpecs(t *testing.T) {
	...
	v := newTestVariantGenerator(t, resp, nil)

	specs, err := v.GenerateSpecs(context.Background(), &productenrich.ProductAnalysis{})
	if err != nil {
...
func TestGenerateSpecs_LLMError_ReturnsError(t *testing.T) {
	v := newTestVariantGenerator(t, "", errors.New("llm down"))

	_, err := v.GenerateSpecs(context.Background(), &productenrich.ProductAnalysis{})
	if err == nil {
...
func TestGenerateVariants_ValidJSON_ParsesVariants(t *testing.T) {
	...
	variants, err := v.GenerateVariants(context.Background(), &productenrich.ProductAnalysis{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(variants) != 1 {
...
func TestGenerateVariants_LLMError_ReturnsError(t *testing.T) {
	...
	_, err := v.GenerateVariants(context.Background(), &productenrich.ProductAnalysis{})
	if err == nil {
		t.Fatal("expected error when LLM fails")
	}
}
...
func TestExtractDimensions_NullResponse_ReturnsEmptyDimensions(t *testing.T) {
	v := newTestVariantGenerator(t, "null", nil)
...
func TestNewVariantGenerator_ValidLLM_Succeeds(t *testing.T) {
	mgr := newMockLLMManager("{}")
	_, err := productenrichenrich.NewVariantGenerator(mgr)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}
