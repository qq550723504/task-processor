...
var installationStatements = []string{`CREATE TABLE public.account_business_profiles (
    ...
    CONSTRAINT account_business_profiles_pkey PRIMARY KEY (organization_id, user_id),
    CONSTRAINT account_business_profiles_organization_id_check CHECK (octet_length(organization_id) BETWEEN 1 AND 128 AND organization_id = btrim(organization_id)),
...
func VerifySchema(ctx context.Context, db *gorm.DB) error {
	...
	sqlDB, err := db.DB()
	if err != nil {
		return fmt.Errorf("get account profile schema database: %w", err)
	}
...
func expectedColumns() []string {
	return []string{"organization_id|character varying|128|NO", "user_id|character varying|128|NO", "user_role|character varying|128|NO", "shop_situation|character varying|128|NO", "factory_situation|character varying|128|NO", "platforms|text||NO", "sites|text||NO", "shop_type|character varying|128|NO", "services|text||NO", "updated_at|timestamp with time zone||NO"}
}
