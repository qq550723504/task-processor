...
func VerifyPermissions(ctx context.Context, db *gorm.DB) error {
 ...
 FROM pg_roles WHERE rolname=current_user`).Scan(&safe).Error
	...
	for _, table := range []string{"referral_codes", "registration_intents", "referral_relations", "referral_receipts", "registration_admission_buckets"} {
		for _, permission := range []string{"SELECT", "INSERT", "UPDATE", "DELETE", "TRUNCATE", "REFERENCES", "TRIGGER"} {
			want := permission == "SELECT" || permission == "INSERT" || (table == "registration_admission_buckets" && (permission == "UPDATE" || permission == "DELETE"))
			var have bool
 ...
 ('public.registration_intents'::regclass,'public.referral_codes'::regclass,'public.referral_relations'::regclass,'public.referral_receipts'::regclass,'public.registration_admission_buckets'::regclass)
 ...
 (attrelid='public.registration_admission_buckets'::regclass OR (attrelid='public.registration_intents'::regclass AND attname IN ('state','ciphertext','lease_until'))))`).Scan(&invalid).Error
