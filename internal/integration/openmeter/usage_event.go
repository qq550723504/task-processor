...
const (
	MetricListingKitGenerationsSucceeded Metric = "listingkit_generations_succeeded"
...
func validateMetricQuantity(metric Metric, quantity string) (string, error) {
	if metric == MetricListingKitGenerationsSucceeded || metric == MetricProductImageJobsSucceeded || metric == MetricSheinDraftsSucceeded || metric == MetricSheinPublishesSucceeded {
		if quantity != "1" {
...
func isKnownMetric(metric Metric) bool {
	return metric == MetricListingKitGenerationsSucceeded || metric == MetricProductImageJobsSucceeded || metric == MetricSheinDraftsSucceeded || metric == MetricSheinPublishesSucceeded || metric == MetricStorageBytesCurrent
}
