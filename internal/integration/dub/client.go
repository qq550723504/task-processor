...
func (c *Client) UpsertPartner(ctx context.Context, input PartnerInput) (Partner, error) {
	...
	payload := struct {
		...
		Email: input.Email, Name: input.Name, Username: input.Username,
		TenantID: input.ExternalID, Country: input.Country,
	}
...
// CreatePartnerLink creates a referral link for a Dub Partner Program member.
// Dub associates the link with the partner through tenantId. The adapter sends
// only request fields currently documented for /partners/links.
func (c *Client) CreatePartnerLink(ctx context.Context, input PartnerLinkInput) (Link, error) {
	...
	payload := struct {
		...
		TenantID: input.ExternalPartnerID,
