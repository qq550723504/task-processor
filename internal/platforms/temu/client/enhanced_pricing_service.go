// Package temu 提供增强的TEMU核价服务，保持原有接口和逻辑。
package client
...
import (
	...
	"task-processor/internal/common/pricing/core"

	"github.com/sirupsen/logrus"
)

// EnhancedPricingDecisionService 增强的核价决策服务，保持原有逻辑
type EnhancedPricingDecisionService struct {
...
// NewEnhancedPricingDecisionService 创建增强的核价决策服务
func NewEnhancedPricingDecisionService(managementClient *management.ClientManager, tenantID, storeID int64) *EnhancedPricingDecisionService {
	service := &EnhancedPricingDecisionService{
		managementClient: managementClient,
		tenantID:         tenantID,
		storeID:          storeID,
		pricingEngine:    core.NewPricingEngine(managementClient),
		logger: logrus.WithFields(logrus.Fields{
			"component": "EnhancedPricingDecisionService",
			"tenantID":  tenantID,
			"storeID":   storeID,
		}),
	}
...
// MakeDecision 对单个商品做出核价决策 (保持原有接口和逻辑)
func (s *EnhancedPricingDecisionService) MakeDecision(item *Sku, storeId int64) (*PricingDecision, error) {
	...
	// 从上架记录映射获取原始成本价 (使用通用引擎)
	mapping, err := s.pricingEngine.GetProductImportMapping(item.SkuSN, storeId)
	if err != nil {
		decision.Action = DecisionSkip
	...
	// 计算原始成本价 (使用通用引擎)
	originCostPrice := s.pricingEngine.CalculateOriginCostPrice(mapping, item.SupplierPrice)
	...
	pricingRule, err := s.pricingEngine.GetPricingRule(s.tenantID, storeId)
	if err != nil {
...
// MakeDecisionForSalesBoost 对销量提升场景的商品做出核价决策 (保持原有接口和逻辑)
func (s *EnhancedPricingDecisionService) MakeDecisionForSalesBoost(goods *SalesBoostGoods, sku *SalesBoostSku, storeId int64) (*PricingDecision, error) {
	decision := &PricingDecision{}
	...
	// 从上架记录映射获取原始成本价 (使用通用引擎)
	mapping, err := s.pricingEngine.GetProductImportMapping(sku.OutSkuSN, storeId)
	if err != nil {
		decision.Action = DecisionSkip
		decision.Reason = fmt.Sprintf("获取上架记录失败: %v", err)
	...
	// 计算原始成本价 (使用通用引擎)
	originCostPrice := s.pricingEngine.CalculateOriginCostPrice(mapping, currentSupplierPrice)
	if originCostPrice <= 0 {
		decision.Action = DecisionSkip
		decision.Reason = "无法计算原始成本价"
	...
	// 决策逻辑：如果目标价格能满足利润率要求，则接受；否则根据店铺配置决定 (保持原有逻辑)
	minAcceptablePrice := originCostPrice * profitRate
...
// 以下方法保持原有逻辑不变
func (s *EnhancedPricingDecisionService) loadStoreConfig() error {
...
func (s *EnhancedPricingDecisionService) isRebargainEnabled() bool {
...
func (s *EnhancedPricingDecisionService) getPriceRejectStrategy() string {
