...
func NewTemuTaskSubmitter(workerPool processor.WorkerPool) *TemuTaskSubmitter {
...
// SubmitTask 提交任务
func (s *TemuTaskSubmitter) SubmitTask(taskData string) error {
	var task types.Task
	...
	// 提交到工作池
	if err := s.workerPool.Submit(processor.WorkerJob{
	...
	logrus.Infof("[TEMU] ✅ 任务已提交到工作池: ID=%s, ProductID=%s", task.ID, task.ProductID)
	return nil
}
...
func (s *TemuTaskSubmitter) GetQueueStats() processor.QueueStats {
	if s.workerPool == nil {
