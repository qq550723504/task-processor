// Package handlers 提供属性验证阶段，负责验证属性的有效性和完整性
package handlers
...
func NewPropertyValidationStage(logger *logrus.Entry) *PropertyValidationStage {
	return &PropertyValidationStage{
		...
		ruleEngine:        NewValidationRuleEngine(logger),
		logger:            logger,
	}
}

// NewPropertyValidationStageWithEngine 创建带验证引擎的属性验证阶段
func NewPropertyValidationStageWithEngine(logger *logrus.Entry, ruleEngine *ValidationRuleEngine) *PropertyValidationStage {
	return &PropertyValidationStage{
		BasePropertyStage: NewBasePropertyStage("属性验证阶段", 200),
...
func (s *PropertyValidationStage) Process(ctx *PropertyContext) error {
	...
	if s.ruleEngine == nil {
		s.ruleEngine = NewValidationRuleEngine(s.logger)
	}
...
func (s *PropertyValidationStage) SetRuleEngine(ruleEngine *ValidationRuleEngine) {
