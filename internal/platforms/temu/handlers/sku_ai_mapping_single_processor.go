// Package handlers 提供TEMU平台的AI SKU映射单批次处理功能
package handlers
...
func (vp *SkuVariantProcessor) generateAISkuMappingSingleBatch(temuCtx *temucontext.TemuTaskContext, variants []*model.Product) (*types.AISkuMappingResponse, error) {
	...
	// 创建属性提取管理器
	attributeManager := NewAttributeExtractionManager(vp.logger)
...
func (vp *SkuVariantProcessor) getTemuSpecProperties(temuCtx *temucontext.TemuTaskContext) []types.TemplateRespGoodsSpecProperty {
	...
	if templateInfo, exists := GetTemplateInfoFromContext(temuCtx); exists {
		...
		if len(temuSpecProperties) == 0 {
			if userInputSpecs, exists := GetUserInputParentSpecListFromContext(temuCtx); exists {
