...
func NewProductSubmitHandler(mappingClient management_api.ProductImportMappingAPI) *ProductSubmitHandler {
	...
	baseHandler.logger = logger.GetGlobalLogger("temu.handlers.product_submit")

	validator := NewProductSubmitValidator(baseHandler.logger)
	fixer := NewProductSubmitFixer(baseHandler.logger, validator)
