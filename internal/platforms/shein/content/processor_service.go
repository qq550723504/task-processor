...
func (s *SensitiveWordService) removeSensitiveWordsAndBrandsWithContext(ctx *model.TaskContext, text string) string {
	if text == "" {
		return text
	}
...
func (s *SensitiveWordService) processMultiLanguageNamesWithBrandsAndContext(ctx *model.TaskContext, nameList []product.LanguageContent) int {
	if nameList == nil {
		return 0
	}
...
func (s *SensitiveWordService) processSKCData(ctx *model.TaskContext, skcList []product.SKC) int {
	if skcList == nil {
		return 0
	}
...
func (s *SensitiveWordService) removeContextBrandWords(ctx *model.TaskContext, text string) string {
