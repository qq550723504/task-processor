...
import (
	...
	sheinapi "task-processor/internal/platforms/shein/client/api"
...
func (h *EnhancedAutoPricingHandler) Start(ctx context.Context, interval time.Duration) {
	// 使用统一的goroutine管理器
	goroutineManager := goroutine.NewGoroutineManager(ctx, logrus.WithField("component", "enhanced_pricing_handler"))

	goroutineManager.StartPeriodic("enhanced_auto_pricing", interval, func(ctx context.Context) error {
		// 执行自动核价
