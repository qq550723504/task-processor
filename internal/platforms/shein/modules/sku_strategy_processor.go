// Package modules 提供SHEIN平台SKU策略处理功能
package modules
...
func NewSKUStrategyProcessor(variantMatcher *VariantMatcher) *SKUStrategyProcessor {
...
func (p *SKUStrategyProcessor) BuildSingleSKU(ctx *TaskContext, req SKUBuildRequest) ([]product.SKU, error) {
	...
	sku, err := p.creator.CreateSKU(ctx, SKUCreationParams{
...
func (p *SKUStrategyProcessor) BuildMultipleSKUs(ctx *TaskContext, req SKUBuildRequest) ([]product.SKU, error) {
...
func (p *SKUStrategyProcessor) buildSKUListForMultipleVariants(ctx *TaskContext, variantInfoMap map[string]variantInfo, req SKUBuildRequest) ([]product.SKU, error) {
	// 结果列表
