...
import (
	...
	"task-processor/internal/common/pricing"
...
func (h *IntegratedAutoPricingHandler) Start(ctx context.Context, interval time.Duration) {
	// 使用统一的goroutine管理器
	goroutineManager := goroutine.NewGoroutineManager(ctx, h.logger)

	goroutineManager.StartPeriodic("integrated_auto_pricing", interval, func(ctx context.Context) error {
