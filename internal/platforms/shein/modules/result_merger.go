// Package modules 提供SHEIN平台结果合并功能
package modules
...
func (m *ResultMerger) MergeResults(results []ResultSaleAttribute) ResultSaleAttribute {
	if len(results) == 0 {
...
func (m *ResultMerger) mergeSaleAttributes(results []ResultSaleAttribute) []SaleAttribute {
	// 使用map去重，key为属性ID
	attrMap := make(map[int]*SaleAttribute)
	...
	for _, result := range results {
		for _, attr := range result.SaleAttributes {
			if existingAttr, exists := attrMap[attr.AttrID]; exists {
				...
				// 复制属性
				newAttr := SaleAttribute{
					AttrID:    attr.AttrID,
					AttrValue: make([]AttributeValue, len(attr.AttrValue)),
				}
...
func (m *ResultMerger) mergeAttributeValues(existing, new []AttributeValue) []AttributeValue {
	// 使用map去重，key为属性值的value
	valueMap := make(map[string]AttributeValue)
...
// mergeVariants 合并变体
func (m *ResultMerger) mergeVariants(results []ResultSaleAttribute) []Variant {
...
func (m *ResultMerger) ValidateMergedResult(result ResultSaleAttribute) []string {
	var issues []string
