...
func (h *SaleAttributePreparationHandler) prepareProductsData(ctx *TaskContext) []map[string]string {
	return h.productDataPreparer.PrepareProductsData(ctx)
}

// buildAttributeMetadata 构建属性元数据（保持原有接口）
func (h *SaleAttributePreparationHandler) buildAttributeMetadata(ctx *TaskContext, importanceCalc *AttributeImportanceCalculator) []AttributeMetadata {
...
func (h *SaleAttributePreparationHandler) filterVariantsByRules(ctx *TaskContext) {
	h.variantFilter.FilterVariantsByRules(ctx)
}

// filterVariantsByRulesAfterGeneration 在生成销售属性之后过滤变体（保持原有接口）
func (h *SaleAttributePreparationHandler) filterVariantsByRulesAfterGeneration(ctx *TaskContext, saleAttributeData *ResultSaleAttribute) {
	h.variantFilter.FilterVariantsByRulesAfterGeneration(ctx, saleAttributeData)
}

// buildCompactProductContext 构建精简的产品上下文信息（保持原有接口）
func (h *SaleAttributePreparationHandler) buildCompactProductContext(ctx *TaskContext) string {
	return h.contextBuilder.BuildCompactProductContext(*ctx.AmazonProduct, *ctx.Variants)
}

// buildExtraContext 构建额外上下文信息（保持原有接口）
func (h *SaleAttributePreparationHandler) buildExtraContext(ctx *TaskContext, productsData []ProductVariantData) string {
...
func (h *SaleAttributePreparationHandler) buildUserPrompt(ctx *TaskContext, request *GenerationRequest) string {
...
func buildAttributeMetadata(ctx *TaskContext, importanceCalc *AttributeImportanceCalculator) []AttributeMetadata {
...
func filterVariantsByRules(ctx *TaskContext) {
	defaultPreparationHandler.filterVariantsByRules(ctx)
}

// filterVariantsByRulesAfterGeneration 全局函数，保持向后兼容
func filterVariantsByRulesAfterGeneration(ctx *TaskContext, saleAttributeData *ResultSaleAttribute) {
...
func buildCompactProductContext(ctx *TaskContext) string {
	return defaultPreparationHandler.buildCompactProductContext(ctx)
}

// buildExtraContext 全局函数，保持向后兼容
func buildExtraContext(ctx *TaskContext, productsData []ProductVariantData) string {
...
func buildUserPrompt(ctx *TaskContext, request *GenerationRequest) string {
