...
func (p *SaleAttributeBatchProcessor) ProcessInBatches(ctx *TaskContext, request *GenerationRequest, batchSize int) ResultSaleAttribute {
	...
	var allSaleAttributes []ResultAttribute
	...
	for batchIndex := 0; batchIndex < totalBatches; batchIndex++ {
		...
		var batchProductsData []ProductVariantData
		if start < len(productsData) {
			...
			batchProductsData = []ProductVariantData{}
		}
