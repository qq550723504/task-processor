...
func (h *AttributeStrategyHandler) DetermineAttributeStrategy(saleAttributeData ResultSaleAttribute, config AttributePriorityConfig, attributeTemplates *attribute.AttributeTemplateInfo) AttributeStrategy {
	...
	// 1. 寻找主要属性（用于SKC分组）- 基于重要性优先级，不限制变体数量
	...
	for _, priorityID := range config.SKCPrimaryAttributePriority {
		for _, attr := range saleAttributeData.SaleAttributes {
			if attr.AttrID == priorityID && len(attr.AttrValue) > 0 {
				// 验证该属性值在变体中实际存在
				if h.validateAttributeInVariants(attr.AttrID, attr.AttrValue, saleAttributeData.Variants) {
					primaryAttr = attr
	...
	// 如果没有找到合适的主要属性，检查是否只有尺寸属性而没有颜色属性
	if !foundPrimaryAttr {
		logrus.Infof("未找到合适的主要属性，检查是否只有尺寸属性")
		...
		if hasSizeAttribute && !h.hasColorAttribute(saleAttributeData) {
			logrus.Infof("检测到只有尺寸属性而没有颜色属性，创建默认颜色属性")
			...
			secondaryAttr = sizeAttribute
			strategyType = "color_size" // 设置策略类型为颜色_尺寸
			logrus.Infof("创建默认主要属性: ID=%d, Value=%s", primaryAttr.AttrID, primaryAttr.AttrValue[0].Value)

			// 直接返回策略，因为我们已经确定了主要和次要属性
...
func (h *AttributeStrategyHandler) validateAttributeInVariants(attrID int, attrValues []AttributeValue, variants []Variant) bool {
	// 获取属性名称的可能变体
...
func (h *AttributeStrategyHandler) getAttributeNameVariations(attrID int) []string {
	...
	default:
		return []string{"size", "Size", "尺寸", "尺码"} // 默认假设是尺寸
	}
}
