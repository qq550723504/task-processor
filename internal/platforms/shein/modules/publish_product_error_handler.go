...
func (h *PublishProductErrorHandler) HandlePublishResponse(ctx *TaskContext, response *product.SheinResponse) error {
	// 检查发布结果
...
func (h *PublishProductErrorHandler) autoReplaceSensitiveWordsAndResubmit(ctx *TaskContext, results []PreValidResult) bool {
...
func (h *PublishProductErrorHandler) getSensitiveWordService() *SensitiveWordService {
	// 创建简化的敏感词服务实例
...
func (h *PublishProductErrorHandler) parsePreValidResult(preValidResult interface{}) ([]PreValidResult, error) {
...
func (h *PublishProductErrorHandler) hasValidationError(results []PreValidResult) bool {
...
func (h *PublishProductErrorHandler) formatValidationErrors(results []PreValidResult) string {
...
func (h *PublishProductErrorHandler) isSpecificationError(results []PreValidResult) bool {
...
func (h *PublishProductErrorHandler) isDuplicateSKUError(results []PreValidResult) bool {
	for _, result := range results {
		// 检查错误消息中是否包含"卖家SKU重复"
		for _, message := range result.Messages {
...
func (h *PublishProductErrorHandler) isQuantityTypeError(results []PreValidResult) bool {
...
func (h *PublishProductErrorHandler) autoFixQuantityTypeAndResubmit(ctx *TaskContext, results []PreValidResult) bool {
	...
	for skcIndex, skc := range ctx.ProductData.SKCList {
		for skuIndex, sku := range skc.SKUS {
			if sku.QuantityInfo != nil {
				...
				// 使用SKUUtils的修正逻辑
				skuUtils := NewSKUUtils()
				correctedQuantityType, correctedQuantity := skuUtils.correctQuantityTypeAndValue(
