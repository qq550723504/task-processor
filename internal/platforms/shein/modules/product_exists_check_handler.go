...
// Handle 执行产品存在性检查
func (h *ProductExistsCheckHandler) Handle(ctx *TaskContext) error {
	...
	logrus.Info("✅ 产品存在性检查完成")
	return nil
}

// checkMainProduct 检查主产品是否已上架
func (h *ProductExistsCheckHandler) checkMainProduct(ctx *TaskContext, mappingClient management_api.ProductImportMappingAPI) error {
	...
	if err != nil {
		...
		// 检查失败可能是网络问题，可重试
		return NewRetryableError("检查主产品是否已上架失败", err)
	}

	if exists {
		logrus.Warnf("⚠️ 主产品 %s 已经上架过，跳过本次上架", ctx.Task.ProductID)
		return NewNonRetryableError(fmt.Sprintf("主产品 %s 已经上架过", ctx.Task.ProductID), nil)
	}
...
func (h *ProductExistsCheckHandler) checkVariantProducts(ctx *TaskContext, mappingClient management_api.ProductImportMappingAPI) error {
	if ctx.Variants == nil || len(*ctx.Variants) == 0 {
		logrus.Debug("无变体产品，跳过变体检查")
		return nil
	}

	logrus.Infof("开始检查 %d 个变体产品...", len(*ctx.Variants))

	for i, variant := range *ctx.Variants {
		if variant.Asin == "" {
			logrus.Debugf("变体[%d/%d] ASIN为空，跳过", i+1, len(*ctx.Variants))
			continue
		}

		if err := h.checkSingleVariant(ctx, mappingClient, variant, i+1, len(*ctx.Variants)); err != nil {
			// 单个变体检查失败不影响整体流程，记录日志后继续
			logrus.Warnf("变体[%d/%d] %s 检查失败: %v", i+1, len(*ctx.Variants), variant.Asin, err)
		}
	}
...
func (h *ProductExistsCheckHandler) checkSingleVariant(ctx *TaskContext, mappingClient management_api.ProductImportMappingAPI, variant model.Product, index, total int) error {
	req := &management_api.ProductImportMappingCheckReqDTO{
		StoreId:   ctx.Task.StoreID,
		Platform:  ctx.Task.Platform,
		Region:    ctx.Task.Region,
	...
	exists, err := mappingClient.CheckProductExists(req)
	if err != nil {
		logrus.Errorf("❌ 检查变体[%d/%d] %s 是否已上架失败: %v", index, total, variant.Asin, err)
		return err
	}

	if exists {
		logrus.Warnf("⚠️ 变体[%d/%d] %s 已经上架过", index, total, variant.Asin)
		// 标记该变体已被筛选掉
		ctx.SetVariantFiltered(variant.Asin, true, fmt.Sprintf("变体 %s 已经上架过", variant.Asin))
	} else {
		logrus.Debugf("✅ 变体[%d/%d] %s 未上架", index, total, variant.Asin)
	}
