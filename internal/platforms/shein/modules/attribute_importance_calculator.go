// Package modules 提供SHEIN平台属性重要性计算功能
package modules
...
func (c *AttributeImportanceCalculator) EnhanceAttributeDataWithTemplateInfo(attributeData []GenerateAttribute, attributeTemplates *attribute.AttributeTemplateInfo) []EnhancedGenerateAttribute {
	...
	for _, attr := range attributeData {
		...
		if template, exists := templateMap[attr.AttrID]; exists {
			// 使用统一的重要性计算函数
			importanceResult := c.CalculateAttributeImportance(template)
			...
			// 检查依赖关系
			deps := c.getAttributeDependencies(attr.AttrID)
...
func (c *AttributeImportanceCalculator) CalculateAttributeImportance(attribute *attribute.AttributeInfo) AttributeImportanceResult {
	...
	if c.IsKeyPrimaryAttribute(attribute.AttributeName, attribute.AttributeNameEn) {
...
func (c *AttributeImportanceCalculator) IsKeyPrimaryAttribute(attributeName string, attributeNameEn string) bool {
...
func (c *AttributeImportanceCalculator) getAttributeDependencies(attrID int) []int {
