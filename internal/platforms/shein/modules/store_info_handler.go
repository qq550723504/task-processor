// Package modules 提供SHEIN平台的处理模块
package modules
...
func (h *StoreInfoHandler) Handle(ctx *TaskContext) error {
	...
	if ShouldSkipForAmazon(ctx) {
		h.logger.Info("Amazon任务，跳过店铺信息获取")
		return nil
	}
...
func (h *StoreInfoHandler) validateStoreStatus(ctx *TaskContext, storeInfo *management_api.StoreRespDTO) error {
...
func (h *StoreInfoHandler) cleanupStoreResources(ctx *TaskContext) {
	// 通过内存管理器清理相关缓存
...
// pauseStore 暂停店铺
func (h *StoreInfoHandler) pauseStore(ctx *TaskContext, reason string) {
	if ctx.MemoryManager != nil {
