...
// GetDefaultTimeRange 获取默认的时间范围（最近三个月，符合API限制）
func (h *TimeHelper) GetDefaultTimeRange() (startTime, endTime string) {
	now := time.Now()
	end := now.Add(24 * time.Hour) // 明天
	start := end.AddDate(0, -3, 0) // 明天往前推三个月（符合API限制）
...
func (h *TimeHelper) ValidateTimeRange(startTime, endTime string) error {
	...
	// 检查时间范围是否超过三个月（API限制）
	maxDuration := 90 * 24 * time.Hour // 90天
...
func (h *TimeHelper) GetMaxAllowedTimeRange(endTime time.Time) (startTime time.Time) {
