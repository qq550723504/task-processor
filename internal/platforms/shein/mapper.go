...
func MapToProductData(sheinProduct *SheinProductResponse, tenantID, storeID int64) (*api.ProductDataDTO, error) {
	now := time.Now()

	// 解析发布时间，失败时设置为 nil
	var publishTime *time.Time
	if sheinProduct.PublishTime != "" {
		parsedTime, err := ParseTime(sheinProduct.PublishTime)
		if err != nil {
	...
	if sheinProduct.FirstShelfTime != "" {
		parsedTime, err := ParseTime(sheinProduct.FirstShelfTime)
		if err != nil {
			logrus.WithFields(logrus.Fields{
				"spu_code":         sheinProduct.SpuCode,
