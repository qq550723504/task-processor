...
func NewPriceManager(baseClient *BaseAPIClient, errorHandler *APIErrorHandler) *PriceManager {
	return &PriceManager{
...
func (m *PriceManager) QueryPrice(spuName string) (*product.PriceQueryResponse, error) {
	url := fmt.Sprintf("%s%s", m.baseClient.GetBaseURL(), queryPriceEndpoint)
...
func (m *PriceManager) QueryCostPrice(spuName string, skcNameList []string) (*product.CostPriceQueryResponse, error) {
	url := fmt.Sprintf("%s%s", m.baseClient.GetBaseURL(), queryCostPriceEndpoint)
