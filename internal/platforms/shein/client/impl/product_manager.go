...
// OffShelf 产品下架
func (m *ProductManager) OffShelf(request *product.ShelfOperateRequest) error {
	url := fmt.Sprintf("%s%s", m.baseClient.GetBaseURL(), operateShelfStatusEndpoint)
	...
	// 统一错误处理
	return m.errorHandler.ProcessAPIResponse(&result, "0", url, "产品下架失败")
}

// OnShelf 产品上架
func (m *ProductManager) OnShelf(request *product.ShelfOperateRequest) error {
	url := fmt.Sprintf("%s%s", m.baseClient.GetBaseURL(), operateShelfStatusEndpoint)
	...
	// 统一错误处理
	return m.errorHandler.ProcessAPIResponse(&result, "0", url, "产品上架失败")
}
