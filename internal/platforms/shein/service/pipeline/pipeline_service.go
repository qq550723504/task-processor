...
import (
	...
	"task-processor/internal/platforms/shein/service/common/info"
	"task-processor/internal/platforms/shein/service/content"
	"task-processor/internal/platforms/shein/service/product"
	"task-processor/internal/platforms/shein/service/product/attribute"
	"task-processor/internal/platforms/shein/service/product/attribute/sale"
	"task-processor/internal/platforms/shein/service/product/build"
	"task-processor/internal/platforms/shein/service/product/image"
...
func CreateTaskProcessingPipeline(processor *SheinProcessor, cfg *config.Config) *Pipeline {
	...
	// 获取店铺信息
	pipeline.AddHandler(info.NewStoreInfoHandler(storeClient))
	// 重新上架任务处理器
	//pipeline.AddHandler(modules.NewReListingHandler())
	// 获取原始数据（支持从Amazon爬虫抓取，使用共享的Amazon处理器）
	pipeline.AddHandler(data.NewRawJsonDataHandler(processor.GetManagementClient().GetRawJsonDataAdapter(), cfg, processor.amazonProcessor, processor.rabbitmqClient))
	...
	pipeline.AddHandler(data.NewSubmitRawJsonDataHandler(processor.GetManagementClient().GetRawJsonDataAdapter(), cfg, processor.amazonProcessor, processor.rabbitmqClient))
	// 初始化产品数据
	pipeline.AddHandler(product.NewInitProductDataHandler())
	// 获取店铺ID
	pipeline.AddHandler(info.NewSupplierInfoHandler(storeClient))
	// 处理店铺ID
	...
	pipeline.AddHandler(data.NewVariantJsonDataHandler(processor.GetManagementClient().GetRawJsonDataAdapter(), &cfg.Amazon, processor.amazonProcessor, processor.rabbitmqClient))
	// 提交变体原始JSON数据到服务器缓存（使用公共缓存逻辑）
	pipeline.AddHandler(data.NewSubmitVariantRawJsonDataHandler(processor.GetManagementClient().GetRawJsonDataAdapter(), cfg, processor.amazonProcessor, processor.rabbitmqClient))
	// 重新应用筛选规则到变体
	...
	// 获取仓库信息
	pipeline.AddHandler(info.NewWarehouseInfoHandler())
	// 翻译标题描述
	pipeline.AddHandler(translate.NewTranslateHandler(openaiConfig))
	// 设置站点信息
	pipeline.AddHandler(info.NewSiteInfoHandler())
	// 获取属性模板
