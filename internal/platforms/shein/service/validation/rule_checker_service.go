...
func (c *FilterRuleChecker) CheckPriceRange(filterRule *api.FilterRuleRespDTO, priceValue float64) error {
	...
	if filterRule.PriceMin != nil {
		if priceValue < *filterRule.PriceMin {
			return shein_model.NewFilteredError(fmt.Sprintf("产品价格(%.2f)低于筛选规则最低价格(%.2f)", priceValue, *filterRule.PriceMin))
		}
	}
	...
	if filterRule.PriceMax != nil {
		if priceValue > *filterRule.PriceMax {
			return shein_model.NewFilteredError(fmt.Sprintf("产品价格(%.2f)高于筛选规则最高价格(%.2f)", priceValue, *filterRule.PriceMax))
		}
	}
...
func (c *FilterRuleChecker) CheckInventory(filterRule *api.FilterRuleRespDTO, inventory int) error {
	...
	if inventory < *filterRule.StockMin {
		return shein_model.NewFilteredError(fmt.Sprintf("产品库存(%d)低于筛选规则最低库存(%d)", inventory, *filterRule.StockMin))
	}
	return nil
}
...
func (c *FilterRuleChecker) CheckDeliveryTime(filterRule *api.FilterRuleRespDTO, deliveryTime int) error {
	...
	if deliveryTime > *filterRule.DeliveryTimeMax*24 {
		return shein_model.NewFilteredError(fmt.Sprintf("产品发货时效(%d小时)超过筛选规则最大时效(%d小时)", deliveryTime, *filterRule.DeliveryTimeMax))
	}
	return nil
}
...
func (c *FilterRuleChecker) CheckRating(filterRule *api.FilterRuleRespDTO, rating float64) error {
	...
	if rating < *filterRule.RatingMin {
		return shein_model.NewFilteredError(fmt.Sprintf("产品评论星级(%.1f)低于筛选规则最低星级(%.1f)", rating, *filterRule.RatingMin))
	}
	return nil
}
...
func (c *FilterRuleChecker) CheckReviewCount(filterRule *api.FilterRuleRespDTO, reviewCount int) error {
	...
	if reviewCount < *filterRule.ReviewCountMin {
		return shein_model.NewFilteredError(fmt.Sprintf("产品评论数量(%d)低于筛选规则最低评论数量(%d)", reviewCount, *filterRule.ReviewCountMin))
	}
	return nil
}
...
func (c *FilterRuleChecker) CheckFulfillmentType(filterRule *api.FilterRuleRespDTO, amazonProduct *model.Product) error {
	...
	// 空指针检查
	if amazonProduct == nil {
		return shein_model.NewFilteredError("产品信息为空，无法校验配送方式")
	}
	...
	case "FBA":
		if !isFBA {
			return shein_model.NewFilteredError(fmt.Sprintf("产品配送方式不符合要求：规则要求FBA配送，但产品为FBM配送 (ships_from: %s)", amazonProduct.ShipsFrom))
		}
	case "FBM":
		if isFBA {
			return shein_model.NewFilteredError(fmt.Sprintf("产品配送方式不符合要求：规则要求FBM配送，但产品为FBA配送 (ships_from: %s)", amazonProduct.ShipsFrom))
		}
	case "AMZ":
		if !isAMZ {
			return shein_model.NewFilteredError(fmt.Sprintf("产品配送方式不符合要求：规则要求亚马逊自营，但卖家为 %s", amazonProduct.SellerName))
		}
	default:
