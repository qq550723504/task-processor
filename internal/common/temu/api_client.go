// Package temu 提供TEMU平台API客户端核心功能
package temu
...
func NewAPIClient(tenantID, storeID int64, managementClient *management.ClientManager) *APIClient {
	...
	// 初始化各个管理器
	apiClient.httpManager = NewHTTPManager(config, apiClient.proxyURL, logger)
	apiClient.authManager = NewAuthManager(apiClient.cookieManager, logger)
	apiClient.productAPI = NewProductAPI(apiClient, logger)
