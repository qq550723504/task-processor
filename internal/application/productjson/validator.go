...
type InputValidator interface {
	// Validate 验证输入数据
	Validate(ctx context.Context, input *domain.ParsedInput) (*domain.ValidationResult, error)
	// ValidateImages 验证图片数据
	ValidateImages(ctx context.Context, imageURLs []string) (*domain.ImageValidation, error)
	// ValidateText 验证文本数据
	ValidateText(ctx context.Context, text string) (*domain.TextValidation, error)
	// ValidateScrapedData 验证抓取数据
	ValidateScrapedData(ctx context.Context, data *domain.ScrapedData) (*domain.ScrapedDataValidation, error)
}
...
// Validate 验证输入数据
func (v *inputValidator) Validate(ctx context.Context, input *domain.ParsedInput) (*domain.ValidationResult, error) {
	if input == nil {
	...
	if err != nil {
		...
		if v.metrics != nil {
			v.metrics.RecordCacheOperation("validation_issue", domain.SeverityError)
		}
	} else {
...
// ValidateImages 验证图片数据
func (v *inputValidator) ValidateImages(ctx context.Context, imageURLs []string) (*domain.ImageValidation, error) {
	validation := &domain.ImageValidation{
		TotalCount:  len(imageURLs),
		ValidImages: make([]domain.ImageInfo, 0),
	}
	...
	resultChan := make(chan domain.ImageInfo, len(imageURLs))
...
func (v *inputValidator) validateSingleImage(ctx context.Context, imageURL string) domain.ImageInfo {
	// 尝试从缓存获取
	if v.cache != nil {
...
func (v *inputValidator) cacheImageInfo(ctx context.Context, imageURL string, info *domain.ImageInfo) {
	if v.cache != nil {
...
// ValidateText 验证文本数据
func (v *inputValidator) ValidateText(ctx context.Context, text string) (*domain.TextValidation, error) {
	validation := &domain.TextValidation{
		Length: len(text),
	}
...
// ValidateScrapedData 验证抓取数据
func (v *inputValidator) ValidateScrapedData(ctx context.Context, data *domain.ScrapedData) (*domain.ScrapedDataValidation, error) {
	if data == nil {
		return nil, fmt.Errorf("scraped data cannot be nil")
	}

	validation := &domain.ScrapedDataValidation{
		HasTitle:       data.Title != "",
