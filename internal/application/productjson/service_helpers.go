...
// parseInput 解析输入
func (s *productService) parseInput(ctx context.Context, task *domain.Task) (*domain.ParsedInput, error) {
...
func (s *productService) validateAndSelectStrategy(ctx context.Context, task *domain.Task, parsedInput *domain.ParsedInput) (domain.ProcessingStrategy, error) {
	if s.inputValidator == nil || s.qualityScorer == nil || s.strategySelector == nil {
		...
		return domain.StrategyFull, nil
	}
	...
	// 如果策略是拒绝，生成改进建议并返回错误
	if strategy == domain.StrategyReject {
...
func (s *productService) handleRejection(ctx context.Context, task *domain.Task, validationResult *domain.ValidationResult, qualityScore float64) (domain.ProcessingStrategy, error) {
...
// analyzeProduct 分析产品
func (s *productService) analyzeProduct(ctx context.Context, task *domain.Task, parsedInput *domain.ParsedInput) (*domain.ProductAnalysis, error) {
	...
	return &domain.ProductAnalysis{
		Representation: &domain.ProductRepresentation{
...
func (s *productService) generateProductJSON(ctx context.Context, task *domain.Task, analysis *domain.ProductAnalysis) (*domain.ProductJSON, error) {
...
func (s *productService) validateResult(ctx context.Context, task *domain.Task, parsedInput *domain.ParsedInput, productJSON *domain.ProductJSON) {
...
func (s *productService) buildRejectionMessage(validation *domain.ValidationResult, suggestion *domain.EnhancementSuggestion) string {
