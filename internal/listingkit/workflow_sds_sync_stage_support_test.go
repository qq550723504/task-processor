...
func TestSDSRenderedImageSetIncompleteWhenSDSReturnsTooFewImages(t *testing.T) {
	t.Parallel()
	...
	if !sdsRenderedImageSetIncomplete(summary, options) {
		t.Fatal("expected an incomplete render when SDS returns fewer images than requested")
	}
}

func TestSDSRenderedImageSetIncompleteSkipsCompleteSDSSet(t *testing.T) {
	t.Parallel()
	...
	if sdsRenderedImageSetIncomplete(summary, options) {
		t.Fatal("did not expect an incomplete render when SDS returns a complete image set")
	}
}
