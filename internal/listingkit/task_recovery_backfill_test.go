...
func TestBackfillRetryableFailuresReclassifiesKnownCreditErrors(t *testing.T) {
	...
	retryableTask := &Task{
		...
		Request:   &GenerateRequest{ProductKey: "test-product", TenantID: "tenant-backfill", Platforms: []string{"shein"}, Text: "retryable"},
	...
	nonRetryableTask := &Task{
		...
		TenantID:  "tenant-backfill",
		Status:    core.TaskStatusFailed,
		Request:   &GenerateRequest{ProductKey: "test-product", TenantID: "tenant-backfill", Platforms: []string{"shein"}, Text: "non-retryable"},
