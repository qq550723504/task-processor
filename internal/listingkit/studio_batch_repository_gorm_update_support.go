...
func (r *GormStudioBatchRepository) UpdateStudioBatch(ctx context.Context, batch *StudioBatchRecord) error {
	...
	result := applyStudioBatchAccessScope(r.db.WithContext(ctx), ctx).
		...
		Updates(map[string]any{
			...
			"image_strategy":                 row.ImageStrategy,
			"product_image_count":            row.ProductImageCount,
			"product_image_prompt":           row.ProductImagePrompt,
			"product_image_prompts":          row.ProductImagePrompts,
...
func (r *GormStudioBatchRepository) ClaimStudioMaterializedDesignBackgroundRemoval(ctx context.Context, design *StudioMaterializedDesignRecord) (bool, error) {
	if design == nil {
		return false, nil
	}
	row := *design
