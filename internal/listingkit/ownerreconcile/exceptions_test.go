...
func TestPostgresExceptionStoreLoadsActiveRows(t *testing.T) {
	...
	query := `SELECT table_name, tenant_fingerprint, candidate_fingerprint, report_fingerprint, reason, row_count
...
ORDER BY table_name, tenant_fingerprint, candidate_fingerprint`
	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{
		"table_name", "tenant_fingerprint", "candidate_fingerprint", "report_fingerprint", "reason", "row_count",
	}).AddRow("listing_store", "sha256:000000000000", "sha256:111111111111", "648cdfab03c4", "legacy orphaned owner", int64(7)))
	...
	if len(got) != 1 || got[0].Table != "listing_store" || got[0].ReportFingerprint != "648cdfab03c4" || got[0].Rows != 7 {
...
ORDER BY table_name, tenant_fingerprint, candidate_fingerprint`
	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{
		"table_name", "tenant_fingerprint", "candidate_fingerprint", "report_fingerprint", "reason", "row_count",
	}).AddRow("", "sha256:000000000000", "sha256:111111111111", "648cdfab03c4", "legacy orphaned owner", int64(7)))
...
func TestPostgresExceptionStoreRejectsUnapprovedReportFingerprint(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
...
ORDER BY table_name, tenant_fingerprint, candidate_fingerprint`
	mock.ExpectQuery(regexp.QuoteMeta(query)).WillReturnRows(sqlmock.NewRows([]string{
		"table_name", "tenant_fingerprint", "candidate_fingerprint", "report_fingerprint", "reason", "row_count",
	}).AddRow("listing_store", "sha256:000000000000", "sha256:111111111111", "deadbeefdead", "legacy orphaned owner", int64(7)))

	_, err = NewPostgresExceptionStore(db).ListActive(context.Background())
	if err == nil || !strings.Contains(err.Error(), "exception registry") {
		t.Fatalf("err = %v, want unapproved report fingerprint rejection", err)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Fatal(err)
	}
