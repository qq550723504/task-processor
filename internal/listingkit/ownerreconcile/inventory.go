...
func buildOwnerReconciliationInventory() []TableSpec {
	...
	inventory = append(inventory, legacyImportMappingSpec())
...
func legacySimpleSpec(table string) TableSpec {
	return TableSpec{
		...
		CandidatePolicy: CandidatePolicyCreatorFirst,
...
func legacyImportMappingSpec() TableSpec {
	return TableSpec{
		Table:           "listing_product_import_mapping",
		TenantDomain:    TenantDomainLegacyNumeric,
		CandidatePolicy: CandidatePolicyStoreOnly,
		UpdateQuery:     fmt.Sprintf("WITH target AS (SELECT row.id FROM listing_product_import_mapping AS row LEFT JOIN listing_store AS store ON store.id = row.store_id AND store.tenant_id = row.tenant_id WHERE row.tenant_id = $2 AND NULLIF(BTRIM(row.owner_user_id::text), '') IS NULL AND %s ORDER BY row.id LIMIT $5) UPDATE listing_product_import_mapping AS row SET owner_user_id = $1 FROM target WHERE row.id = target.id", optionalCandidatePair("store.creator", "$3", "store.created_by", "$4")),
		UpdateLimitArg:  5,
		Query: `SELECT row.tenant_id::text, store.creator::text AS store_creator, store.created_by::text AS store_created_by, COUNT(*)::bigint AS row_count
FROM listing_product_import_mapping AS row
LEFT JOIN listing_store AS store ON store.id = row.store_id AND store.tenant_id = row.tenant_id
WHERE NULLIF(BTRIM(row.owner_user_id::text), '') IS NULL
GROUP BY row.tenant_id, store.creator, store.created_by`,
		Columns: []string{"tenant_id", "store_creator", "store_created_by", "row_count"},
		CandidateColumns: []CandidateColumn{
			{Name: "store_creator", Source: "store"},
			{Name: "store_created_by", Source: "store_created_by"},
		},
...
func nativeSpec(table string) TableSpec {
	return TableSpec{
		...
		CandidatePolicy: CandidatePolicySystemOwned,
