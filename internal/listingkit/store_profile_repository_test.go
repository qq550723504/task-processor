...
func TestGormStoreRoutingSettingsRepositoryPersistsByTenant(t *testing.T) {
	t.Parallel()
	...
	repo := NewGormStoreRoutingSettingsRepository(db)
	ctx := openaiclient.WithIdentity(context.Background(), openaiclient.Identity{TenantID: "303"})

	defaults, err := repo.GetByTenant(ctx, 303)
	if err != nil {
		t.Fatalf("get defaults: %v", err)
	}
	if defaults.SelectionStrategy != "manual" || !defaults.AllowFallback || !defaults.AllowManualOverride {
		t.Fatalf("defaults = %+v, want manual fallback-enabled defaults", defaults)
	}

	saved, err := repo.Upsert(ctx, &ListingKitStoreRoutingSettings{
		TenantID:            303,
	...
	if err != nil {
		t.Fatalf("upsert routing settings: %v", err)
	}
	if saved.FallbackStoreID != 901 || saved.SelectionStrategy != "priority" {
		t.Fatalf("saved = %+v, want persisted routing settings", saved)
	}

	current, err := repo.GetByTenant(ctx, 303)
	if err != nil {
		t.Fatalf("get current settings: %v", err)
	}
	if current.FallbackStoreID != 901 || current.AllowManualOverride {
		t.Fatalf("current = %+v, want updated routing settings", current)
	}
}
