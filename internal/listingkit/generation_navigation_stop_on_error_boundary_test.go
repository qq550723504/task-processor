...
func TestGenerationNavigationStopOnErrorUsesDomainDirectly(t *testing.T) {
	...
	if strings.Contains(content, "func buildGenerationNavigationDispatchStopOnError(") {
		t.Fatal("generation_navigation_target_dispatch_support.go should not keep the forwarding wrapper")
	}
	if !strings.Contains(content, "listinggeneration.NavigationDispatchStopOnError(") {
