...
func TestRetryStudioBatchDesignBackgroundRemovalRejectsPersistedPendingDesign(t *testing.T) {
	t.Parallel()
	...
	if _, err := svc.RetryStudioBatchDesignBackgroundRemoval(ctx, "batch-1", &RetryStudioBatchDesignBackgroundRemovalRequest{DesignIDs: []string{"design-1"}}); err == nil {
		t.Fatal("RetryStudioBatchDesignBackgroundRemoval() error = nil, want pending validation error")
	}
	if providerCalls != 0 {
