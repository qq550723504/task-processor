...
func TestAIClientSettingsFacadeFileOwnsRootDelegates(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_ai_client_settings_admin_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_ai_client_settings_admin_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_ai_client_settings_admin_facade.go should contain %q", needle)
		}
	}
...
func TestSheinSettingsFacadeFileOwnsRootDelegates(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_shein_settings_admin_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_shein_settings_admin_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_shein_settings_admin_facade.go should contain %q", needle)
		}
	}
...
func TestSheinSubmissionEventsFacadeFileOwnsRootDelegate(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_shein_submission_event_list_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_shein_submission_event_list_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_shein_submission_event_list_facade.go should contain %q", needle)
		}
	}
...
func TestTaskSDSBaselineFacadeFileOwnsWarmDelegate(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_task_sds_baseline_warm_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_task_sds_baseline_warm_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_task_sds_baseline_warm_facade.go should contain %q", needle)
		}
	}
