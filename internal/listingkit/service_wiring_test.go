...
func TestSubmitTemporalFacadeFileOwnsRootDelegates(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_shein_publish_temporal_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_shein_publish_temporal_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_shein_publish_temporal_facade.go should contain %q", needle)
		}
	}
...
func TestSheinPricingFacadeFileOwnsRootDelegate(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_shein_pricing_preview_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_shein_pricing_preview_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_shein_pricing_preview_facade.go should contain %q", needle)
		}
	}
