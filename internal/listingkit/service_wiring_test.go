...
func TestSubmitRuntimeContextFilesUseExplicitResolverSeam(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			file: "service_submit_context_helpers.go",
			needles: []string{
...
func TestStoreProfileFacadeFileOwnsRootDelegates(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_store_profile_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_store_profile_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_store_profile_facade.go should contain %q", needle)
		}
	}
...
func TestSubmitContextHelpersFileOwnsRootHelpers(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_submit_context_helpers.go")
	if err != nil {
		t.Fatalf("ReadFile(service_submit_context_helpers.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_submit_context_helpers.go should contain %q", needle)
		}
	}
...
func TestTaskSDSBaselineFacadeFileOwnsWarmDelegate(t *testing.T) {
	...
	facadeSrc, err := os.ReadFile("service_task_sds_baseline_facade.go")
	if err != nil {
		t.Fatalf("ReadFile(service_task_sds_baseline_facade.go) error = %v", err)
	}
	facadeContent := string(facadeSrc)
	...
	for _, needle := range []string{
		...
		if !strings.Contains(facadeContent, needle) {
			t.Fatalf("service_task_sds_baseline_facade.go should contain %q", needle)
		}
	}
