...
func BuildSettingsHealth(inputs SettingsHealthInputs) SettingsHealthPage {
	items := []SettingsHealthItem{
		...
		runtimeProbeHealthItem("sds.session", "SDS 登录态", mergeLegacyProbe(inputs.Probes.SDSLogin, inputs.SDSProbeConfigured), []string{"SDS 属性补全", "SDS 商品库和 Studio"}, "接入 SDS 登录态探针或补齐目标租户和店铺对应的 SDS 登录态。"),
