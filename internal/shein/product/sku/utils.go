...
func (u *SKUUtils) BuildQuantityInfo(params shein.SKUCreationParams) *product.QuantityInfo {
	...
	if err != nil {
		logger.GetGlobalLogger("shein/product").Warnf("获取正确的数量单位失败: %v，使用原始值: %d", err, quantityUnit)
	} else {
		// 如果用户提供的单位与规则不符，使用正确的单位
		if quantityUnit != correctUnit {
			logger.GetGlobalLogger("shein/product").Warnf("数量单位不符合规则，quantityType=%d 应使用单位=%d，当前单位=%d，已自动修正",
	...
	if err := validator.ValidateQuantityMapping(quantityType, quantityUnit); err != nil {
		logger.GetGlobalLogger("shein/product").Errorf("数量信息验证失败: %v", err)
	}

	// 验证数量值的合理性
	if err := validator.ValidateQuantity(quantity, quantityType); err != nil {
		logger.GetGlobalLogger("shein/product").Errorf("数量值验证失败: %v", err)
	}
...
func (u *SKUUtils) BuildSKUImageInfoForMultiPiece(ctx *shein.TaskContext, params shein.SKUCreationParams) *product.ImageInfo {
	...
	for _, imageURL := range sourceImages {
		...
		if err != nil {
			logger.GetGlobalLogger("shein/product").Warnf("上传多件商品SKU图片失败，ASIN: %s, 原始URL: %s, 错误: %v", params.ASIN, imageURL, err)
			continue
		}
	...
	if len(skuImages) == 0 {
		logger.GetGlobalLogger("shein/product").Warnf("多件商品ASIN %s 的所有图片上传都失败了", params.ASIN)
		return nil
	}

	logger.GetGlobalLogger("shein/product").Infof("为多件商品ASIN %s 成功构建了 %d 张SKU图片", params.ASIN, len(skuImages))
...
func (u *SKUUtils) CorrectQuantityTypeAndValue(quantityType, quantity int, asin string) (int, int) {
	...
	logger.GetGlobalLogger("shein/product").Warnf("ASIN %s 的数量设置不符合规则: quantityType=%d, quantity=%d", asin, quantityType, quantity)

	// 修正策略1：如果是同款多件(2)但数量为1，改为单品(1)
	if quantityType == 2 && quantity == 1 {
		logger.GetGlobalLogger("shein/product").Infof("ASIN %s: 将同款多件(quantityType=2, quantity=1) 修正为单品(quantityType=1, quantity=1)", asin)
		return 1, 1
	}

	// 修正策略2：如果是多套(4)但数量为1，改为单套(3)
	if quantityType == 4 && quantity == 1 {
		logger.GetGlobalLogger("shein/product").Infof("ASIN %s: 将多套(quantityType=4, quantity=1) 修正为单套(quantityType=3, quantity=1)", asin)
		return 3, 1
	}
	...
	if (quantityType == 1 || quantityType == 3) && quantity > 1 {
		if quantityType == 1 {
			logger.GetGlobalLogger("shein/product").Infof("ASIN %s: 将单品(quantityType=1, quantity=%d) 修正为同款多件(quantityType=2, quantity=%d)", asin, quantity, quantity)
			return 2, quantity
		} else {
			logger.GetGlobalLogger("shein/product").Infof("ASIN %s: 将单套(quantityType=3, quantity=%d) 修正为多套(quantityType=4, quantity=%d)", asin, quantity, quantity)
			return 4, quantity
		}
	}
	...
	if (quantityType == 2 || quantityType == 4) && quantity < 2 {
		logger.GetGlobalLogger("shein/product").Infof("ASIN %s: 将数量从 %d 修正为 2 (quantityType=%d 要求数量≥2)", asin, quantity, quantityType)
		return quantityType, 2
	}

	// 默认策略：如果无法修正，使用最安全的单品设置
	logger.GetGlobalLogger("shein/product").Warnf("ASIN %s: 无法智能修正，使用默认单品设置(quantityType=1, quantity=1)", asin)
	return 1, 1
}
