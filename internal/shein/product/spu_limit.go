...
func (h *SpuLimitHandler) submitRemainingQuota(ctx *shein.TaskContext, quota int) {
	...
	if client == nil {
		logger.GetGlobalLogger("shein/product").Warn("每日上架数量客户端未初始化，无法提交剩余发品额度")
		return
	}

	success, err := client.SetRemainingListingQuota(ctx.Task.TenantID, ctx.Task.StoreID, quota)
	if err != nil {
		logger.GetGlobalLogger("shein/product").Warnf("提交剩余发品额度失败: %v", err)
	} else if !success {
		logger.GetGlobalLogger("shein/product").Warnf("提交剩余发品额度返回失败: storeID=%d, quota=%d", ctx.Task.StoreID, quota)
		// 不影响主流程，继续执行
	} else {
		logger.GetGlobalLogger("shein/product").Infof("成功提交剩余发品额度: %d", quota)
	}
}
...
func (h *SpuLimitHandler) Handle(ctx *shein.TaskContext) error {
	...
	if err != nil {
		logger.GetGlobalLogger("shein/product").Infof("获取发品额度失败: %v\n", err)
		return err
	}

	// 输出日志信息
	logger.GetGlobalLogger("shein/product").Infof("发品额度信息: 可用状态=%d, 可用额度=%d, 已使用额度=%d, 剩余额度=%d\n",
	...
	if spuLimitCount.QuotaRemain < 1 {
		logger.GetGlobalLogger("shein/product").Warnf("发品额度不足，终止任务: 可用状态=%d, 剩余额度=%d",
