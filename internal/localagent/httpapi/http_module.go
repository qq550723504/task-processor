...
func BuildModule(service *localagent.Service) *BuildResult {
	handler := NewHandler(service)
	return &BuildResult{Handler: handler, Module: NewHTTPModule(handler)}
}

func NewHTTPModule(handler *Handler) module.Module { return routeModule{handler: handler} }

func (m routeModule) Name() string { return ModuleName }

func (m routeModule) Enabled(*config.Config) bool {
	return m.handler != nil && m.handler.service != nil
}
