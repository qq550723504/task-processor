...
func TestMatchSubmitVariantOptionIndexFallsBackByColorThenGlobalIndex(t *testing.T) {
	...
	if got := MatchSubmitVariantOptionIndex(input, "white", &SubmitDraftSKU{}, 0); got != 1 {
		t.Fatalf("color match index = %d, want 1", got)
	}
	if got := MatchSubmitVariantOptionIndex(input, "", &SubmitDraftSKU{}, 1); got != 1 {
...
func TestResolveSubmitBaseSKUAndDiscriminator(t *testing.T) {
	...
	draft := &SubmitDraftSKU{Attributes: map[string]string{"Color": "black", "Size": "L"}}
...
func TestBuildStudioVariantSKUIncludesDiscriminatorWhenRequired(t *testing.T) {
	t.Parallel()

	got := BuildStudioVariantSKU("MG8014186001", "D7E68190", "v101 / task", true, map[string]int{})

	if got != "MG8014186001-V101-TASK-D7E68190" {
		t.Fatalf("BuildStudioVariantSKU() = %q, want normalized discriminator and style suffix", got)
	}
}

func TestBuildStudioVariantSKUDeduplicatesSeenBaseCandidate(t *testing.T) {
	t.Parallel()

	seen := map[string]int{}
	first := BuildStudioVariantSKU("SKU", "STYLE", "V1", false, seen)
	second := BuildStudioVariantSKU("SKU", "STYLE", "V1", false, seen)
	third := BuildStudioVariantSKU("SKU", "STYLE", "V1", false, seen)

	if first != "SKU-STYLE" || second != "SKU-V1-STYLE" || third != "SKU-V1-STYLE-2" {
		t.Fatalf("dedupe sequence = %q, %q, %q", first, second, third)
	}
}
