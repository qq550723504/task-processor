...
// SubmitDraftSKU is the neutral draft SKU input needed for variant SKU policy.
type SubmitDraftSKU struct {
	Attributes map[string]string
}

// MatchSubmitVariantOptionIndex matches a draft SKU to an SDS variant and returns its index.
func MatchSubmitVariantOptionIndex(input *SubmitVariantContext, draftSKCValue string, draftSKU *SubmitDraftSKU, globalIndex int) int {
...
func ResolveSubmitBaseSKU(input *SubmitVariantContext, draftSKU *SubmitDraftSKU, match *SubmitVariantOption, oldSKU string) string {
	if match != nil {
...
func ResolveSubmitVariantDiscriminator(input *SubmitVariantContext, draftSKU *SubmitDraftSKU, match *SubmitVariantOption, matchedIndex, globalIndex int, taskDiscriminator string) string {
	base := ""
	if match != nil {
...
// NormalizeSubmitVariantDiscriminator normalizes a variant discriminator to its SKU-safe form.
func NormalizeSubmitVariantDiscriminator(value string) string {
	return normalizeSubmitVariantDiscriminator(value)
}
