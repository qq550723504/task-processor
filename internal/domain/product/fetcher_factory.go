// Package product 提供产品获取器工厂
package product

import (
	"fmt"

	"task-processor/internal/core/config"
	"task-processor/internal/crawler/amazon"
	"task-processor/internal/domain/model"

	"github.com/sirupsen/logrus"
)

// FetcherType 获取器类型
type FetcherType string

const (
	// LocalFetcher 本地获取器（使用本地Amazon处理器）
	LocalFetcher FetcherType = "local"
	// DistributedFetcher 分布式获取器（使用分布式爬虫集群）
	DistributedFetcher FetcherType = "distributed"
)

// ProductFetcherInterface 产品获取器接口
type ProductFetcherInterface interface {
	FetchProduct(req *FetchRequest) (*model.Product, error)
	CacheProduct(req *FetchRequest, product *model.Product) error
	CacheVariants(req *FetchRequest, variants []*model.Product) error
	GetStats() map[string]interface{}
}

// FetcherFactory 获取器工厂
type FetcherFactory struct {
	logger *logrus.Entry
}

// NewFetcherFactory 创建获取器工厂
func NewFetcherFactory() *FetcherFactory {
	return &FetcherFactory{
		logger: logrus.WithField("component", "FetcherFactory"),
	}
}

// CreateFetcher 创建产品获取器
func (f *FetcherFactory) CreateFetcher(
	fetcherType FetcherType,
	rawJsonDataClient RawJsonDataClient,
	amazonConfig *config.AmazonConfig,
	amazonProcessor *amazon.AmazonProcessor,
	rabbitmqURL string,
) (ProductFetcherInterface, error) {

	switch fetcherType {
	case LocalFetcher:
		f.logger.Info("创建本地产品获取器")
		return NewProductFetcher(rawJsonDataClient, amazonConfig, amazonProcessor), nil

	case DistributedFetcher:
		f.logger.Info("创建分布式产品获取器")
		if rabbitmqURL == "" {
			return nil, fmt.Errorf("分布式获取器需要RabbitMQ URL")
		}
		return NewDistributedProductFetcher(rawJsonDataClient, amazonConfig, rabbitmqURL)

	default:
		return nil, fmt.Errorf("不支持的获取器类型: %s", fetcherType)
	}
}

// CreateFetcherFromConfig 根据配置创建获取器
func (f *FetcherFactory) CreateFetcherFromConfig(
	cfg *config.Config,
	rawJsonDataClient RawJsonDataClient,
	amazonProcessor *amazon.AmazonProcessor,
) (ProductFetcherInterface, error) {

	// 检查是否启用分布式爬虫
	if cfg.RabbitMQ != nil && cfg.RabbitMQ.Enabled && cfg.RabbitMQ.URL != "" {
		f.logger.Info("配置启用分布式爬虫，创建分布式获取器")
		return f.CreateFetcher(
			DistributedFetcher,
			rawJsonDataClient,
			&cfg.Amazon,
			amazonProcessor,
			cfg.RabbitMQ.URL,
		)
	}

	// 默认使用本地获取器
	f.logger.Info("使用本地获取器")
	return f.CreateFetcher(
		LocalFetcher,
		rawJsonDataClient,
		&cfg.Amazon,
		amazonProcessor,
		"",
	)
}

// GetRecommendedFetcher 获取推荐的获取器类型
func (f *FetcherFactory) GetRecommendedFetcher(cfg *config.Config) FetcherType {
	// 如果配置了RabbitMQ，推荐使用分布式获取器
	if cfg.RabbitMQ != nil && cfg.RabbitMQ.Enabled && cfg.RabbitMQ.URL != "" {
		return DistributedFetcher
	}

	// 默认推荐本地获取器
	return LocalFetcher
}
