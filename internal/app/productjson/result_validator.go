...
type ResultValidator interface {
	...
	ValidateResult(ctx context.Context, input *domain.ParsedInput, result *domain.ProductJSON) (*domain.ResultValidation, error)
	...
	CheckCompleteness(result *domain.ProductJSON) (*domain.CompletenessReport, error)
}
...
// ValidateResult 验证生成结果
func (v *resultValidator) ValidateResult(ctx context.Context, input *domain.ParsedInput, result *domain.ProductJSON) (*domain.ResultValidation, error) {
	if input == nil {
	...
	validation := &domain.ResultValidation{
		IsValid: true,
...
// CheckCompleteness 检查完整性
func (v *resultValidator) CheckCompleteness(result *domain.ProductJSON) (*domain.CompletenessReport, error) {
	if result == nil {
		return nil, fmt.Errorf("result cannot be nil")
	}
