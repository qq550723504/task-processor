...
type QualityScorer interface {
	...
	CalculateScore(ctx context.Context, validation *domain.ValidationResult) (float64, error)
	// ScoreImages 评估图片质量
	ScoreImages(ctx context.Context, imageValidation *domain.ImageValidation) (float64, error)
	// ScoreText 评估文本质量
	ScoreText(ctx context.Context, textValidation *domain.TextValidation) (float64, error)
	// ScoreScrapedData 评估抓取数据质量
	ScoreScrapedData(ctx context.Context, scrapedValidation *domain.ScrapedDataValidation) (float64, error)
}
...
// CalculateScore 计算总体质量评分
func (s *qualityScorer) CalculateScore(ctx context.Context, validation *domain.ValidationResult) (float64, error) {
	if validation == nil {
...
// ScoreImages 评估图片质量
func (s *qualityScorer) ScoreImages(ctx context.Context, imageValidation *domain.ImageValidation) (float64, error) {
...
// ScoreText 评估文本质量
func (s *qualityScorer) ScoreText(ctx context.Context, textValidation *domain.TextValidation) (float64, error) {
...
// ScoreScrapedData 评估抓取数据质量
func (s *qualityScorer) ScoreScrapedData(ctx context.Context, scrapedValidation *domain.ScrapedDataValidation) (float64, error) {
