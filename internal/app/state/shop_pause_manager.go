...
func (m *ShopPauseManager) PauseShopForDuration(tenantID, shopID int64, reason string, duration time.Duration) {
	...
	logrus.Infof("店铺暂停 %v: 租户=%d, 店铺=%d, 原因=%s", duration, tenantID, shopID, reason)
}
...
func (m *ShopPauseManager) PauseShopUntilEndOfDay(tenantID, shopID int64, reason string) {
	...
	if endOfDay.Sub(now) < time.Minute {
		logrus.Infof("距离当天结束不足1分钟，跳过暂停设置，等待新的一天重置配额: 租户=%d, 店铺=%d", tenantID, shopID)
		return
	}

	m.pauseShopInternal(tenantID, shopID, reason, endOfDay, "quota_limit")
	logrus.Infof("店铺暂停到当日结束: 租户=%d, 店铺=%d, 原因=%s, 恢复时间=%s", tenantID, shopID, reason, endOfDay.Format("2006-01-02 15:04:05"))
}
...
func (m *ShopPauseManager) PauseShopForAuthExpired(tenantID, shopID int64, reason string) {
	...
	m.pauseShopInternal(tenantID, shopID, reason, resumeAt, "auth_expired")
	logrus.Infof("店铺因认证过期暂停: 租户=%d, 店铺=%d, 原因=%s", tenantID, shopID, reason)
}
...
func (m *ShopPauseManager) pauseShopInternal(tenantID, shopID int64, reason string, resumeAt time.Time, pauseType string) {
	...
	if m.storeClient != nil {
		logrus.Infof("正在设置店铺 %d 的暂停状态，类型=%s，恢复时间: %s", shopID, pauseType, resumeAt.Format("2006-01-02 15:04:05"))
		...
		if err != nil {
			...
			logrus.Infof("成功设置店铺 %d 的暂停状态 (类型: %s)", shopID, pauseType)
		} else {
...
func (m *ShopPauseManager) ResumeShop(tenantID, shopID int64) {
	...
	// 释放锁后再调用API
	if m.storeClient != nil {
		logrus.Infof("正在恢复店铺 %d 的暂停状态...", shopID)
		success, err := m.storeClient.SetStorePauseStatus(shopID, false, "")
		if err != nil {
			logrus.Errorf("恢复店铺 %d 的暂停状态失败: %v", shopID, err)
		} else if success {
			logrus.Infof("成功恢复店铺 %d 的暂停状态", shopID)
		} else {
			logrus.Warnf("恢复店铺 %d 的暂停状态返回失败", shopID)
		}
	}

	logrus.Infof("店铺已恢复: 租户=%d, 店铺=%d", tenantID, shopID)
}
...
func (m *ShopPauseManager) IsShopPaused(tenantID, shopID int64) bool {
	...
	if m.storeClient != nil {
		...
		if err != nil {
			logrus.Errorf("获取店铺 %d 的暂停状态失败: %v", shopID, err)
			// 如果接口调用失败，降级到本地内存查询
		} else {
			return isPaused
		}
...
func (m *ShopPauseManager) CleanupExpired() {
	...
	for key, info := range m.pauses {
		...
		if now.After(info.ResumeAt) && info.PauseType == "quota_limit" {
			...
			delete(m.pauses, key)
			logrus.Infof("清理过期的配额限制暂停记录: %s (类型: %s)", key, info.PauseType)
		} else if now.After(info.ResumeAt) && info.PauseType == "auth_expired" {
			// 认证过期类型的暂停不自动清理，记录日志提醒
			logrus.Debugf("跳过认证过期类型的暂停记录清理: %s (需要等待登录成功)", key)
		}
	}
	m.mutex.Unlock()
	...
	if m.storeClient != nil {
		for _, shop := range expiredShops {
			logrus.Infof("正在恢复店铺 %d 的暂停状态...", shop.shopID)
			success, err := m.storeClient.SetStorePauseStatus(shop.shopID, false, "")
			if err != nil {
				logrus.Errorf("恢复店铺 %d 的暂停状态失败: %v", shop.shopID, err)
			} else if success {
				logrus.Infof("成功恢复店铺 %d 的暂停状态", shop.shopID)
			} else {
				logrus.Warnf("恢复店铺 %d 的暂停状态返回失败", shop.shopID)
			}
		}
	}
}
...
func (m *ShopPauseManager) StartCleanupTask(ctx context.Context) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				logrus.Errorf("店铺暂停清理任务goroutine panic: %v", r)
			}
		}()
