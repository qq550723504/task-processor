...
func TestAppServiceAssemblyDoesNotPassBootstrapSharedResources(t *testing.T) {
	...
	for _, marker := range []string{
		...
		if strings.Contains(source, marker) {
			t.Fatalf("app.go mentions %q; app service assembly should pass the narrowest local resource view available", marker)
		}
	}
	...
	for _, marker := range []string{
		...
		"func buildProcessorService(logger *logrus.Logger, resources processorServiceResources)",
		"func buildSchedulerService(logger *logrus.Logger, cfg *config.Config, resources schedulerServiceResources)",
	} {
		if !strings.Contains(source, marker) {
