...
func TestGenerationRecoveryConsumesExistingProofWithoutReserveOrDispatch(t *testing.T) {
	owner, commercial, limits, intent := generationPointDatabases(t)
	r, err := resourceadapter.NewGormImageGenerationRepository(commercial, resourceadapter.TransactionConfig{}, owner, generationPointTestAuth{})
	require.NoError(t, err)
	ctx := context.Background()
	...
	require.NoError(t, err)
	require.True(t, won)
	recovery, err := imageagent.NewGenerationRecovery(owner, r)
	require.NoError(t, err)
	state, err := recovery.ReconcileExistingGeneration(ctx, intent.Identity)
	require.NoError(t, err)
	require.Equal(t, imageagent.GenerationUnknown, state)
	read, err := limits.ReadMonthlyLimit(ctx, intent.Identity.TenantID, intent.MemberID)
	require.NoError(t, err)
	...
	state, err = recovery.ReconcileExistingGeneration(ctx, intent.Identity)
	require.NoError(t, err)
	require.Equal(t, imageagent.GenerationSucceeded, state)
	state, err = recovery.ReconcileExistingGeneration(ctx, intent.Identity)
	require.NoError(t, err)
	require.Equal(t, imageagent.GenerationSucceeded, state)
	read, err = limits.ReadMonthlyLimit(ctx, intent.Identity.TenantID, intent.MemberID)
	...
	require.NoError(t, err)
	require.False(t, won)
	got, err := owner.ReadGenerationFact(ctx, intent.Identity)
	require.NoError(t, err)
	require.Equal(t, "committed", got.Settlement.State)
}
