...
func (s *S3DurableArtifactStore) PreserveSlotArtifacts(ctx context.Context, identity imageagent.SlotExternalEffectIdentity, prepared PreparedSlotArtifacts) error {
	bundle, err := s.encodeRecoveryBundle(identity, prepared)
	if err != nil {
		return err
	}
	sum := sha256.Sum256(bundle)
	object := storage.ImmutableObjectPut{
...
func (s *S3DurableArtifactStore) RecoverSlotArtifacts(ctx context.Context, identity imageagent.SlotExternalEffectIdentity, expected imageagent.StagingManifest) (PreparedSlotArtifacts, error) {
	if err := validateIdentity(identity); err != nil {
		return PreparedSlotArtifacts{}, err
	}
	operationCtx, cancel := context.WithTimeout(ctx, s.operationTimeout)
	defer cancel()
	data, inspection, err := s.uploader.ReadObject(operationCtx, recoveryBundleKey(identity), s.maxRecoveryBundleBytes())
	if err != nil {
		return PreparedSlotArtifacts{}, err
	}
...
func (s *S3DurableArtifactStore) encodeRecoveryBundle(identity imageagent.SlotExternalEffectIdentity, prepared PreparedSlotArtifacts) ([]byte, error) {
	if err := validateIdentity(identity); err != nil {
		return nil, err
	}
	manifest, err := imageagent.NormalizeStagingManifest(prepared.Manifest)
...
func (s *S3DurableArtifactStore) decodeRecoveryBundle(identity imageagent.SlotExternalEffectIdentity, expected imageagent.StagingManifest, data []byte) (PreparedSlotArtifacts, error) {
...
func (s *S3DurableArtifactStore) ensureImmutableObject(ctx context.Context, object storage.ImmutableObjectPut) error {
...
func (s *S3DurableArtifactStore) prevalidateManifestForIdentity(manifest imageagent.StagingManifest, expected imageagent.SlotExternalEffectIdentity) ([]prevalidatedStagedAsset, error) {
	validated, err := s.prevalidateManifest(manifest)
	if err != nil {
		return nil, err
	}
...
func (s *S3DurableArtifactStore) validateRecoveredAssetBytes(asset imageagent.StagedAssetRef, data []byte) error {
	if int64(len(data)) != asset.SizeBytes {
...
func (s *S3DurableArtifactStore) maxRecoveryBundleBytes() int64 {
...
func verifyImmutableInspection(inspection storage.ObjectInspection, object storage.ImmutableObjectPut) error {
