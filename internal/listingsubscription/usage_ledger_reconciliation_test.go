...
func TestUsageLedgerReconciliationDoesNotReportValidLifecycleOrIdempotentReplay(t *testing.T) {
	...
	seedUsageLedgerEntitlement(t, repo, "tenant-17", "listingkit", map[string]int{"listingkit_generations_succeeded": 20})
...
func TestUsageLedgerReconciliationReportsBucketAndOutboxMismatchesWithSafeContext(t *testing.T) {
	...
	if err := db.Model(&usageBucketRow{}).
		Where("tenant_id = ? AND module_code = ? AND period_key = ? AND metric = ?", "tenant-17", "listingkit", "2026-08", "listingkit_generations_succeeded").
...
func TestUsageLedgerReconciliationReportsLifecycleMismatch(t *testing.T) {
	event := usageEventRow{EventID: "event-mismatch", TenantID: "tenant-17", ModuleCode: ModuleListingKit, Metric: usageMetricListingKitGenerationsSucceeded, Quantity: 1, PeriodKey: "2026-08", Status: string(UsageEventReserved)}
	report := reconcileUsageLedgerRows([]usageEventRow{event}, []usageBucketRow{{TenantID: "tenant-17", ModuleCode: ModuleListingKit, PeriodKey: "2026-08", Metric: usageMetricListingKitGenerationsSucceeded, Reserved: 1}}, []usageEventOutboxRow{{EventID: event.EventID, Status: "sent"}})
...
func TestUsageLedgerReconciliationAcceptsCancelledSourceAfterReversal(t *testing.T) {
	source := usageEventRow{EventID: "event-source", TenantID: "tenant-17", ModuleCode: ModuleListingKit, Metric: usageMetricListingKitGenerationsSucceeded, Quantity: 1, PeriodKey: "2026-08", Status: string(UsageEventCommitted)}
	reversal := usageEventRow{EventID: "event-reversal", TenantID: "tenant-17", ModuleCode: ModuleListingKit, Metric: usageMetricListingKitGenerationsSucceeded, Quantity: -1, PeriodKey: "2026-08", Status: string(UsageEventReversed), ReversalOf: source.EventID}
