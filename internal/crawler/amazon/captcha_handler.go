...
func (ch *CaptchaHandler) TryBypassCaptcha(page playwright.Page) error {
	logger.GetGlobalLogger("crawler/amazon").Info("检测到验证码，尝试绕过策略")

	// 策略1: 等待一段时间，有时验证码会自动消失
	logger.GetGlobalLogger("crawler/amazon").Info("策略1: 等待验证码自动消失")
	time.Sleep(5 * time.Second)

	if !ch.checkCaptchaExists(page) {
		logger.GetGlobalLogger("crawler/amazon").Info("✅ 验证码已自动消失")
		return nil
	}

	// 策略2: 尝试刷新页面
	logger.GetGlobalLogger("crawler/amazon").Info("策略2: 刷新页面尝试绕过验证码")
	if _, err := page.Reload(playwright.PageReloadOptions{
		Timeout: playwright.Float(15000),
	}); err != nil {
		logger.GetGlobalLogger("crawler/amazon").Warnf("页面刷新失败: %v", err)
	} else {
		time.Sleep(3 * time.Second)
		if !ch.checkCaptchaExists(page) {
			logger.GetGlobalLogger("crawler/amazon").Info("✅ 刷新页面后验证码消失")
			return nil
		}
	}

	// 策略3: 尝试点击页面其他区域
	logger.GetGlobalLogger("crawler/amazon").Info("策略3: 尝试点击页面其他区域")
	if err := page.Click("body"); err == nil {
		time.Sleep(2 * time.Second)
		if !ch.checkCaptchaExists(page) {
			logger.GetGlobalLogger("crawler/amazon").Info("✅ 点击页面后验证码消失")
			return nil
		}
	}

	// 所有策略都失败
	logger.GetGlobalLogger("crawler/amazon").Warn("❌ 所有验证码绕过策略都失败，需要重建浏览器实例")
...
func (ch *CaptchaHandler) HandleCaptchaWithRetry(page playwright.Page, maxRetries int) error {
	for attempt := 0; attempt < maxRetries; attempt++ {
		logger.GetGlobalLogger("crawler/amazon").Infof("验证码处理尝试 %d/%d", attempt+1, maxRetries)
		...
		if attempt < maxRetries-1 {
			logger.GetGlobalLogger("crawler/amazon").Infof("第 %d 次尝试失败，等待后重试", attempt+1)
