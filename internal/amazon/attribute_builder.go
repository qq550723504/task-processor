...
type AttributeBuilder struct {
	identifierService *service.ProductIdentifierService
...
func NewAttributeBuilder() *AttributeBuilder {
	return &AttributeBuilder{
		identifierService: service.NewProductIdentifierService(),
...
func (ab *AttributeBuilder) BuildAttributes(
	...
	requiredAttrs []schema.AttributeInfo,
	data *ProductData,
	productSchema *schema.ProductTypeSchema,
) map[string]any {
...
func (ab *AttributeBuilder) addBasicAttributes(attrs map[string]any, builder *schema.Builder, data *ProductData, marketplaceID string) {
	attrs["item_name"] = builder.BuildAttribute(
		schema.AttributeInfo{Format: schema.FormatWithLang},
		data.Title,
	)
	...
	attrs["condition_type"] = builder.BuildAttribute(
		schema.AttributeInfo{Format: schema.FormatSimple},
		data.Condition,
	)
...
func (ab *AttributeBuilder) addIdentifierAttributes(attrs map[string]any, data *ProductData, marketplaceID string, productType string) {
...
func (ab *AttributeBuilder) addDetailAttributes(attrs map[string]any, data *ProductData, marketplaceID string) {
	// 描述
	if data.Description != "" {
...
func (ab *AttributeBuilder) addRequiredAttributes(attrs map[string]any, builder *schema.Builder, requiredAttrs []schema.AttributeInfo, data *ProductData, marketplaceID string) {
	for _, attr := range requiredAttrs {
		...
		if attr.Name == "closure" {
			...
			if _, exists := data.Attributes["closure"]; !exists {
				// 使用默认值提供器构建closure属性
				provider := schema.NewDefaultValueProvider(marketplaceID, "en_US", data.Brand)
...
func (ab *AttributeBuilder) addImageAttributes(ctx context.Context, attrs map[string]any, data *ProductData, marketplaceID string, productSchema *schema.ProductTypeSchema) {
...
func (ab *AttributeBuilder) addAdditionalImagesBySchema(attrs map[string]any, imageURLs []string, marketplaceID string, productSchema *schema.ProductTypeSchema) {
...
func (ab *AttributeBuilder) getSupportedImageAttributes(productSchema *schema.ProductTypeSchema) []string {
...
func (ab *AttributeBuilder) addCustomAttributes(attrs map[string]any, data *ProductData, marketplaceID string) {
...
func (ab *AttributeBuilder) ensureCommonAttributes(attrs map[string]any, marketplaceID string, data *ProductData, productSchema *schema.ProductTypeSchema) {
	// 如果没有Schema，不添加额外属性
	if productSchema == nil {
		return
	}
...
func (ab *AttributeBuilder) getDefaultValue(attr schema.AttributeInfo, marketplaceID string) any {
	// 如果有枚举值，使用第一个
...
func (ab *AttributeBuilder) buildPurchasableOfferWithTax(builder *schema.Builder, data *ProductData) any {
	marketplaceID := builder.MarketplaceID()
