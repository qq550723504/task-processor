...
const redisHost = process.env.TASK_PROCESSOR_SDS_REDIS_HOST?.trim() || "127.0.0.1";
const redisPort = Number.parseInt(process.env.TASK_PROCESSOR_SDS_REDIS_PORT?.trim() || "6379", 10);
const redisPassword = process.env.TASK_PROCESSOR_SDS_REDIS_PASSWORD?.trim() || "";
const redisDB = Number.parseInt(process.env.TASK_PROCESSOR_SDS_REDIS_DB?.trim() || "9", 10);
const redisKey = process.env.TASK_PROCESSOR_SDS_REDIS_KEY?.trim() || "sds:auth:global";
...
function buildRedisUrl() {
  if (redisPassword) {
    return `redis://:${encodeURIComponent(redisPassword)}@${redisHost}:${redisPort}/${redisDB}`;
  }
  return `redis://${redisHost}:${redisPort}/${redisDB}`;
}

function buildRedisPayload(state) {
  const payload = {
  ...
  if (state.outToken) {
    payload.out_token = state.outToken;
  }
  if (state.cookies) {
...
async function main() {
  ...
  try {
    ...
    while (Date.now() - startedAt < 90000) {
      ...
      if (
        lastState.token &&
        lastState.merchantId > 0 &&
        lastState.userId > 0 &&
        lastState.href &&
        !lastState.href.includes("/user/login")
      ) {
        const redisPayload = buildRedisPayload(lastState);
        await writeRedis(redisPayload);
        const artifact = await captureArtifacts(page, "success", {
          targetURL,
          redisKey,
          redisPayload,
          lastState,
        });
        console.log(JSON.stringify({ success: true, artifact, redisKey, redisPayload, state: lastState }, null, 2));
        return;
      }
      if (lastState.hasVerifyInput) {
        const artifact = await captureArtifacts(page, "verify-required", {
          targetURL,
          lastState,
          redisKey,
        });
    ...
    const artifact = await captureArtifacts(page, "timeout", {
      targetURL,
      lastState,
