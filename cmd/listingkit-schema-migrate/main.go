...
func main() {
	...
	if err := listingkithttpapi.AutoMigrateListingKitRuntimeSchema(db); err != nil {
		exitf("migrate listingkit schema: %v", err)
	}

	fmt.Printf("listingkit schema migration completed using %s\n", *configPath)
}
