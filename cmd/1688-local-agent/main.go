...
func validateCLIOfferURL(raw string) (string, error) {
	...
	if err != nil || parsed.Scheme != "https" || !strings.EqualFold(parsed.Hostname(), "detail.1688.com") || parsed.Port() != "" || parsed.User != nil || parsed.RawQuery != "" || parsed.ForceQuery || parsed.Fragment != "" || parsed.RawPath != "" || !strings.HasPrefix(parsed.Path, "/offer/") || strings.TrimSpace(localagentURLID(parsed.Path)) == "" {
