...
func (ph *PriceHandler) CalculateDefaultPrice(ctx *pipeline.TaskContext) int {
	...
	if err != nil {
		...
		// 使用默认倍数
		return ph.calculatePriceWithMultiplier(ctx.AmazonProduct, 2.0)
	}
	...
	// 获取产品原始价格（参考SHEIN的GetProductPrice逻辑）
	originalPrice := ph.getSupplierCost(ctx.AmazonProduct)
...
func (ph *PriceHandler) calculatePriceWithMultiplier(product *amazon.Product, multiplier float64) int {
	originalPrice := ph.getSupplierCost(product)
...
// getSupplierCost 获取供应商成本（参考SHEIN的价格获取逻辑）
func (ph *PriceHandler) getSupplierCost(product *amazon.Product) float64 {
	// 检查产品是否为空
	if product == nil {
	...
	// 如果 FinalPrice 为0，则使用 InitialPrice（原价）
