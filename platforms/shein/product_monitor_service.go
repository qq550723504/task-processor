...
func (s *SheinProductMonitorService) getStoreMappings(storeID, tenantID int64) ([]*api.ProductDataDTO, error) {
...
func (s *SheinProductMonitorService) saveAmazonProductData(asin, region string, amazonProduct *amazon.Product, tenantID, storeID int64) {
	...
	_, err = s.rawJsonDataClient.CreateRawJsonData(createReq)
	if err != nil {
...
func (s *SheinProductMonitorService) recordInventoryAndPrice(productId, region string, amazonProduct *amazon.Product) {
	...
	recordID, err := s.inventoryRecordClient.CreateInventoryRecord(recordReq)
	if err != nil {
		logrus.WithError(err).WithFields(logrus.Fields{
			"product_id": productId,
			"region":     region,
