...
func (h *TaskHandler) ProcessTask(ctx context.Context, task modules.Task, pipeline *Pipeline) error {
	...
	taskCtx := &modules.TaskContext{
		...
		ManagementClientMgr: h.managementClientMgr,
	}
...
func (h *TaskHandler) handleAuthenticationExpired(authErr *api.AuthenticationExpiredError, task modules.Task) {
	...
	if h.shouldDeleteCookie(shopID) {
		// 调用API删除过期的Cookie
		storeClient := h.managementClientMgr.GetStoreClient()
		if storeClient != nil {
...
func (h *TaskHandler) updateTaskStatusToAPI(taskID string, status types.TaskStatus, errorMsg string) {
...
func (h *TaskHandler) updateTaskStatusToAPIWithMode(taskID string, status types.TaskStatus, errorMsg string, sync bool) error {
	...
	importTaskClient := h.managementClientMgr.GetImportTaskClient()
