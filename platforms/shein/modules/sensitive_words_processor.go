...
// Handle 执行敏感词处理
func (h *SensitiveWordsProcessor) Handle(ctx *TaskContext) error {
...
func (h *SensitiveWordsProcessor) handleBlock(ctx *TaskContext, foundWords map[string][]string) error {
	...
	return NewFilteredError(fmt.Sprintf("产品包含敏感词: %v", foundWords))
}

// handleClean 清理模式 - 自动替换敏感词
func (h *SensitiveWordsProcessor) handleClean(ctx *TaskContext, foundWords map[string][]string) error {
	logrus.WithFields(logrus.Fields{
		"asin":            ctx.AmazonProduct.Asin,
		"sensitive_words": foundWords,
	}).Info("🔧 检测到敏感词,开始自动清理...")
...
func (h *SensitiveWordsProcessor) handleWarn(ctx *TaskContext, foundWords map[string][]string) error {
	logrus.WithFields(logrus.Fields{
		"asin":            ctx.AmazonProduct.Asin,
		"title":           ctx.AmazonProduct.Title,
		"sensitive_words": foundWords,
