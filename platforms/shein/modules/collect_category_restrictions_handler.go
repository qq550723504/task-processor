...
func (h *CollectCategoryRestrictionsHandler) Handle(ctx *TaskContext) error {
...
func (h *CollectCategoryRestrictionsHandler) collectCategoryRestrictions(ctx *TaskContext) error {
	// 检查必要的上下文信息
	...
	if categoryRestrictionClient == nil {
		// 这是一个程序逻辑错误，不应该发生，不可重试
		return NewNonRetryableError("品类限制集合API客户端不存在", nil)
	}
	...
	for _, result := range errorResults {
		if result.Module == "specification_info" && result.Form == "main_specification" {
			...
			if err != nil {
				...
				return NewRetryableError("提交分类限制错误失败", err)
			} else {
