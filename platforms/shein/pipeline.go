...
func CreateTaskProcessingPipeline(processor *SheinProcessor, cfg *config.Config) *Pipeline {
	...
	// 添加处理步骤
	storeClient := processor.managementClientMgr.GetStoreClient()
	imageDownloder := processor.managementClientMgr.GetImageDownloader()
	...
	pipeline.AddHandler(modules.NewShopClientHandler(
		...
		processor.managementClientMgr.GetStoreClient(),
		processor.memoryManager.CookieManager,
	))
	// 初始化产品数据
	...
	// 验证任务（筛选规则和利润规则）
	pipeline.AddHandler(modules.NewTaskValidatorHandler(processor.managementClientMgr))
	// 应用筛选规则
	...
	pipeline.AddHandler(modules.NewVariantJsonDataHandler(processor.managementClientMgr.GetRawJsonDataClient(), &cfg.Amazon, processor.amazonProcessor))
	// 提交变体原始JSON数据到服务器缓存（使用公共缓存逻辑）
	pipeline.AddHandler(modules.NewSubmitVariantRawJsonDataHandler(processor.managementClientMgr.GetRawJsonDataClient(), &cfg.Amazon, processor.amazonProcessor))
	// 重新应用筛选规则到变体
