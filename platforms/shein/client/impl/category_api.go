...
import (
	"fmt"
	"net/http"
	"task-processor/platforms/shein/client/api"
...
func NewCategoryAPI(baseClient *BaseAPIClient) *CategoryAPI {
	return &CategoryAPI{
...
func (a *CategoryAPI) GetCategory(categoryID int) (*category.CategoryInfo, error) {
	url := fmt.Sprintf("%s%s", a.GetBaseURL(), getCategoryEndpoint)
...
func (a *CategoryAPI) GetCategoryTree() (*category.CategoryTreeResponse, error) {
	url := fmt.Sprintf("%s%s", a.GetBaseURL(), getCategoryTreeEndpoint)
