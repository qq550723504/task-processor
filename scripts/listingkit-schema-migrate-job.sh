...
usage() {
  printf 'Usage: %s --manifest PATH --namespace NAMESPACE --image API_IMAGE\n' "$0" >&2
}
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    ...
    --namespace) [[ "$#" -ge 2 ]] || { usage; exit 2; }; K8S_NAMESPACE="$2"; shift 2 ;;
...
if [[ ! -f "$MANIFEST" ]]; then
  printf 'ListingKit schema migration Job manifest not found: %s\n' "$MANIFEST" >&2
  exit 2
fi
if ! listingkit_is_immutable_image "$IMAGE"; then
  printf 'ListingKit schema migration requires a valid immutable image, got: %s\n' "$IMAGE" >&2
  exit 2
fi
if ! grep -q 'REPLACE_WITH_API_IMAGE' "$MANIFEST"; then
  printf 'ListingKit schema migration manifest is missing the API image placeholder: %s\n' "$MANIFEST" >&2
  exit 2
fi

rendered_manifest="$(mktemp "${TMPDIR:-/tmp}/listingkit-schema-migrate.XXXXXX.yaml")"
trap 'rm -f "$rendered_manifest"' EXIT
sed "s|REPLACE_WITH_API_IMAGE|$IMAGE|g" "$MANIFEST" >"$rendered_manifest"

job_name="$(kubectl create -n "$K8S_NAMESPACE" -f "$rendered_manifest" -o 'jsonpath={.metadata.name}')"
if [[ -z "$job_name" ]]; then
  printf 'kubectl created schema migration Job without returning its name\n' >&2
  exit 1
fi
