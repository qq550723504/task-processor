...
if (([regex]::Matches($renderedText, '(?m)^kind: Ingress$')).Count -ne 1) {
    throw 'staging overlay must render exactly one ingress'
}
if ($renderedText -notmatch 'traefik\.ingress\.kubernetes\.io/router\.middlewares: casdoor-auth-rate-limit@kubernetescrd') {
    throw 'staging ingress is missing the auth rate limit middleware'
}
