...
it("actual client -> Next/Auth.js -> BFF -> mounted Browser -> runtime PostgreSQL",async()=>{
  ...
  const first=await capture1688(intent);expect(first).toMatchObject({outcome:"published",catalogVersion:"1",productKey:"crawler:1688:981645030344",replayed:false});expect(first.operationId).not.toBe(key);expect(first.missingFacts.length).toBeGreaterThan(0);
  expect((await control("observe")).publicFetches).toBe(0);
  const publicResult=await acquire1688({...scope,key:randomUUID(),source:"981645030344"});expect(publicResult.catalogVersion).toBe("2");
  ...
  organization="A";await expect(readBrowserCaptureByKey({...scope,organizationId:"A"},key)).rejects.toMatchObject({code:"ACQUISITION_NOT_FOUND"});
  ...
  const lostKey=randomUUID();await control("drop-next");await expect(capture1688({...intent,key:lostKey})).rejects.toMatchObject({code:"OUTCOME_UNKNOWN"});
