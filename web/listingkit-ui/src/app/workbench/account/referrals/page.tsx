...
export default async function AccountReferralsPage() {
  ...
  if (!identity || !readZitadelServerAccessToken(session) || readZitadelSessionError(session)) redirect("/login?returnTo=%2Fworkbench%2Faccount%2Freferrals");
  return <ReferralsPage mode="overview" expectedUserId={String(identity.userId)} registrationAvailable={await isReferralRegistrationAvailable()} />;
}
