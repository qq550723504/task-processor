...
const order = { order_id: "order-1", organization_id: "org-A", kind: "RESOURCE_PURCHASE", description: "AI 点数 × 3", quote_id: "quote-1", currency: "CNY", total_minor: "3000", status: "FULFILLED", product_kind: "AI_POINT", items: [{ order_item_id: "item-1", product_kind: "AI_POINT", resource_type: "ai_point", resource_quantity: "3", amount_minor: "3000" }], created_at: observedAt, updated_at: observedAt };
...
describe("commercial billing read contracts", () => {
  ...
  it("reads the owner resource product kind on order pages and details", async () => {
    expect(parseCommercialOrderPage(orders)?.items[0]).toMatchObject({ kind: "RESOURCE_PURCHASE", product_kind: "AI_POINT" });
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(Response.json(order)));
