...
describe("completed local preparation projection", () => {
  it("preserves exact source facts with only the approved completion meaning", () => {
    ...
    expect(item).toMatchObject({ source_record_id: id, snapshot_version: source.items[0].snapshot_version, completion_basis: "local_record_committed", work_scope: "general", result: { href: `/workbench/shein-records/${id}/diagnostic` } });
