...
const orderItem = z.object({ order_item_id: id, product_kind: z.enum(["STORE_RENEWAL_PERIOD", "AI_POINT", "DATA_ROW"]), resource_type: z.enum(["store_renewal_period", "ai_point", "data_row"]), resource_quantity: nonnegative, amount_minor: nonnegative }).strict();
const order = z.object({ order_id: id, organization_id: id, kind: z.enum(["WALLET_TOP_UP", "RESOURCE_PURCHASE"]), description: text, quote_id: id.optional(), currency: z.literal("CNY"), total_minor: nonnegative, status: z.enum(["PENDING", "FUNDS_RESERVED", "FULFILLING", "FULFILLED", "CANCELLED", "RECONCILIATION_REQUIRED"]), failure_code: z.enum(["INSUFFICIENT_FUNDS", "RESOURCE_GRANT_REJECTED"]).optional(), wallet_reservation_id: id.optional(), items: z.array(orderItem).max(64), created_at: timestamp, updated_at: timestamp }).strict();
