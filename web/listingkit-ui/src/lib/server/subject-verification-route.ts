...
async function handle(request: NextRequest): Promise<Response> {
  ...
  request.signal.addEventListener("abort",abort,{once:true});const timer=setTimeout(abort,15000);
  ...
  try {
    ...
    const authenticated=serverAuth(async(req: NextRequest & {auth?:unknown})=>{
      ...
      return proxyVerification(req,readZitadelServerAccessToken(req.auth as never),String(identity.userId),dispatch);
    });
