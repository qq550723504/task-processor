...
it("admits a bound empty Agent Start and rejects client authority and stale identity", async () => {
    ...
    for (const request of [req('{"model":"other"}'), req('{"organizationId":"A"}'), req('{}', { "X-Expected-User-ID": "other" }), req('{}', { Origin: "http://evil.test" }), req('{}', { "Idempotency-Key": "" })]) {
