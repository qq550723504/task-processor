import type { SheinStyleGalleryItem } from "@/lib/types/shein-style-gallery";
import type { SheinStudioGeneratedDesign } from "@/lib/types/shein-studio";

export const SHEIN_STUDIO_GALLERY_HANDOFF_STORAGE_KEY =
  "listingkit:shein-studio:gallery-handoff";

const HANDOFF_TTL_MS = 24 * 60 * 60 * 1000;
const WARNING_RATIO_DELTA = 0.15;
const BLOCKING_RATIO_DELTA = 0.25;

export type SheinStudioGalleryHandoff = {
  id: string;
  title: string;
  imageUrl: string;
  source: string;
...
function isHandoffPayload(value: unknown): value is SheinStudioGalleryHandoff {
  if (!value || typeof value !== "object") {
    return false;
  }
  const payload = value as SheinStudioGalleryHandoff;
  return (
...
export function buildSheinStudioGalleryHandoff(
  item: SheinStyleGalleryItem,
  dimensions?: { width?: number; height?: number },
): SheinStudioGalleryHandoff {
  return {
    id: item.id,
    title: item.title,
...
export function saveSheinStudioGalleryHandoff(
  handoff: SheinStudioGalleryHandoff,
  storage: Pick<Storage, "setItem"> | null = safeStorage(),
) {
  storage?.setItem(SHEIN_STUDIO_GALLERY_HANDOFF_STORAGE_KEY, JSON.stringify(handoff));
}

export function consumeSheinStudioGalleryHandoff(
  storage: Pick<Storage, "getItem" | "removeItem"> | null = safeStorage(),
  now: Date = new Date(),
) {
  const raw = storage?.getItem(SHEIN_STUDIO_GALLERY_HANDOFF_STORAGE_KEY);
  if (!raw) {
    return null;
  }
  storage?.removeItem(SHEIN_STUDIO_GALLERY_HANDOFF_STORAGE_KEY);
...
export function galleryHandoffToDesign(
  handoff: SheinStudioGalleryHandoff,
