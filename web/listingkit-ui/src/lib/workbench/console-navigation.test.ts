...
describe("Figma Console navigation contract", () => {
  it("connects all delivered enterprise leaves while referrals remain pending", () => {
    ...
    expect(findConsoleRoute("/workbench/account/referrals")?.node.availability).toBe("unavailable");
  });
  ...
  it("keeps profile, resources and audit connected together while undelivered leaves remain pending", () => {
    ...
    for (const path of ["/workbench/account/referrals"])
  ...
  it("exposes the resource page and its existing source-account breadcrumb while referrals remain pending", () => {
