...
describe("ListingKitAppShell", () => {
  ...
  it("renders second-level menu sections that can expand child links", async () => {
    ...
    expect(screen.getByRole("link", { name: "POD 原图查询" })).toHaveAttribute(
      "href",
      "/listing-kits/shein-pod-image-lookup",
    );
    ...
    expect(screen.getByRole("link", { name: "平台上架统计" })).toHaveAttribute(
      "href",
  ...
  it("hides platform store links for tenant users", async () => {
    ...
    expect(screen.queryByRole("link", { name: "平台店铺管理" })).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "平台上架统计" })).not.toBeInTheDocument();
  });
});
