    ...
    <div className="space-y-3">
      ...
      <div className="space-y-3">
        {items.map((item) => {
          ...
          const showUnsupportedMapping =
            title === "阻断项" && !canAct && Boolean(item.key);
          return (
            <div
            ...
            >
              ...
              {showUnsupportedMapping ? (
                <div className="space-y-1 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
                  <p className="text-xs font-semibold text-amber-800">
                    未支持自动跳转
                  </p>
                  <p className="break-words text-xs leading-5 text-amber-800">
                    原始 key：{item.key}
                  </p>
                  <p className="text-xs leading-5 text-amber-800">
                    请记录这个阻断项 key，并补充 SHEIN readiness 映射后再验收。
                  </p>
                </div>
              ) : null}
            </div>
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white/70 p-4">
      <div className="space-y-3">
        ...
        <ResolutionCacheRow
          title="价格"
          item={resolutionCache?.pricing}
          kind="pricing"
          isClearing={clearingResolutionCacheKind === "pricing"}
          onClear={onClearResolutionCache}
        />
      </div>
    </div>
