    ...
    <section className="rounded-2xl border border-zinc-200 bg-white p-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2" role="group" aria-label="价格模式">
            {(["DISCOUNT", "PROFIT"] as SheinActivityPriceMode[]).map((mode) => (
              <button
                className={
                ...
                key={mode}
                onClick={() =>
                  setForm((current) => ({ ...current, activity_price_mode: mode }))
                }
                type="button"
              >
...
export function isSheinActivityStrategyReady(response?: {
  ...
  return isActivityStrategyInputValid({
    activity_price_mode:
      response.strategy.activity_price_mode === "PROFIT" ? "PROFIT" : "DISCOUNT",
    activity_discount_rate: response.strategy.activity_discount_rate,
    activity_stock_ratio: response.strategy.activity_stock_ratio ?? 0,
...
function strategyToForm(
  ...
  return {
    ...
    activity_min_profit_rate: String(
      strategy.activity_min_profit_rate ?? DEFAULT_FORM.activity_min_profit_rate,
    ),
    fixed_price_adjustment: String(
...
function activityStrategyFormKey(strategy?: SheinActivityStrategyRecord | null) {
  if (!strategy) {
    return "empty";
  }
  return [
...
function isActivityStrategyInputValid(input: SheinUpdateActivityStrategyInput) {
  if (!withinRatio(input.activity_stock_ratio, true)) {
    return false;
  }
