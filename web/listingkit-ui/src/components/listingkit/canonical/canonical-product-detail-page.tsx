...
function CanonicalAttributes({ product }: { product: CanonicalProduct }) {
  ...
  return (
    <Card className="p-5">
      <h2 className="text-base font-semibold text-foreground">属性</h2>
...
function AttributeRow({ name, attr }: { name: string; attr: CanonicalAttribute }) {
  return (
    <div className="rounded-lg border border-border bg-muted px-3 py-2">
      <div className="text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">{name}</div>
      <div className="mt-1 text-sm text-foreground">{attr.value || "空值"}{attr.unit ? ` ${attr.unit}` : ""}</div>
    </div>
  );
}
...
function CanonicalVariants({ product }: { product: CanonicalProduct }) {
  ...
  return (
    <Card className="p-5">
      <h2 className="text-base font-semibold text-foreground">变体</h2>
      <div className="mt-4 overflow-x-auto rounded-lg border border-border">
        <Table className="min-w-[32rem]">
          ...
          <TableBody>
            {variants.length === 0 ? (
              <TableRow><TableCell colSpan={3} className="px-3 py-4 text-muted-foreground">暂无变体</TableCell></TableRow>
            ) : variants.map((variant, index) => (
              <TableRow key={`${variant.sku ?? "variant"}-${index}`}>
                <TableCell className="px-3 py-2 font-mono text-xs text-foreground">{variant.sku || "-"}</TableCell>
                <TableCell className="px-3 py-2 text-foreground">{variant.title || "-"}</TableCell>
                <TableCell className="px-3 py-2 text-foreground">{variant.stock ?? "-"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        {traces.length === 0 ? (
          ...
          <div key={field} className="flex flex-col gap-3 rounded-lg border border-border bg-background px-3 py-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
            <div>
              ...
              {trace.review_reason ? <div className="mt-1 text-xs text-muted-foreground">{trace.review_reason}</div> : null}
            </div>
