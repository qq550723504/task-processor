...
it("actual provider/switcher and #327 client isolate an old organization HTTP response", async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (url: string, init?: RequestInit) => {
    ...
    requests.push(init!);
    return new Headers(init?.headers).get("X-Expected-Organization-ID") === "200" ? late : Response.json({ items: [], next_cursor: null });
  }));
  const view = render(<QueryClientProvider client={client}><WorkbenchContextProvider><OrganizationSwitcher /><SheinRecordListPage available /></WorkbenchContextProvider></QueryClientProvider>);
  ...
  await act(async () => release(Response.json(recordListFixture())));
