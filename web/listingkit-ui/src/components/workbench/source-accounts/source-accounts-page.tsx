...
const manageRoles = new Set(["listingkit_operator", "listingkit_admin", "platform_admin"]);
...
export function SourceAccountsPage() {
  ...
  return <ConsolePage title="源账号" className={styles.page} description="可选企业资源：登记和启停仅管理本系统资源，匿名公开商品采集无需先登记或连接源账号。">
    ...
    {context.isLoading || context.isSwitching ? <ConsoleState kind="loading" title="正在确认当前企业">旧账号和待操作已清除。</ConsoleState>
      ...
      : <ScopedAccounts key={scope} scope={scope} organizationId={org.id} organizationName={org.name} actor={context.user.id} canManage={context.roles.some(role => manageRoles.has(role))} />}
  </ConsolePage>;
}

function ScopedAccounts({ scope, organizationId, organizationName, actor, canManage }: { scope: string; organizationId: string; organizationName: string; actor: string; canManage: boolean }) {
  ...
  async function execute(intent: Intent) {
    ...
    try {
      ...
      setUnknown(null); setSuccess(true); setSelected(result.account.id); setSequence(v => v + 1);
      ...
      if (unknown || !(failure instanceof SourceAccountAPIError) || failure.outcome === "unknown") {
        ...
        setUnknown(null); setError(failure.code);
  ...
  function register() {
    if (!canManage || unknown || busy.current) return;
  ...
  return <div className={styles.stack}>
    ...
    {unknown ? <Card className={styles.panel}><p>原操作结果尚未确定。核实将使用相同操作标识与原始内容；不要重复登记或发起新的启停操作。</p><Button variant="outline" disabled={pending} onClick={() => void execute(unknown)}>核实原操作</Button><p>离开或刷新页面不会把未知结果判为失败；返回后先读取服务端现状，再决定后续操作。</p></Card> : null}
    ...
    {canManage ? <Card className={styles.panel}><h2>登记源账号</h2><form onSubmit={event => { event.preventDefault(); register(); }} className={styles.form}>
      <label htmlFor="source-account-name">显示名称</label><Input id="source-account-name" value={displayName} onChange={event => setDisplayName(event.target.value)} disabled={pending || !!unknown} required maxLength={120} aria-describedby="source-account-name-help" />
      <p id="source-account-name-help">名称最多 120 UTF-8 字节，不含首尾空格或控制字符。平台：1688。</p>
      <Button type="submit" disabled={pending || !!unknown}>登记源账号</Button>
...
function SourceError({ code }: { code: string }) {
  ...
  return <ConsoleState kind="error" title={labels[code] ?? "源账号请求失败"}>未据此推断账号状态。请确认当前身份和企业，再刷新读取；已提交但未确认的操作请核实原操作。</ConsoleState>;
}
