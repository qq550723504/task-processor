...
function ScopedMembers({ scope }: { scope: MemberScope }) {
  ...
  return <div className={styles.page}>
    ...
    {!ready ? authorityError ? <MemberFailure code={authorityError.code} /> : query.isError ? <MemberFailure code={query.error instanceof MemberError ? query.error.code : "DEPENDENCY_UNAVAILABLE"} /> : <ConsoleState kind="loading" title="正在读取成员">正在确认成员目录与当前权限。</ConsoleState> : <>
      ...
      <div className={styles.memberMetrics} aria-label="成员目录摘要"><article><span>成员记录</span><strong>{query.data.total}</strong><small>当前企业成员目录返回的总数</small></article><article><span>当前页成员</span><strong>{query.data.items.length}</strong><small>每页最多显示 20 条记录</small></article><article><span>可分配角色</span><strong>{query.data.assignableRoles.length}</strong><small>由当前组织授权 owner 返回</small></article><article><span>邀请中</span><strong>未提供</strong><small>当前成员目录未返回邀请状态</small></article></div>
