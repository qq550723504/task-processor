...
function ScopedMembers({ scope }: { scope: MemberScope }) {
  ...
  return <div className={styles.page}>
    ...
    {!ready ? authorityError ? <MemberFailure code={authorityError.code} /> : query.isError ? <MemberFailure code={query.error instanceof MemberError ? query.error.code : "DEPENDENCY_UNAVAILABLE"} /> : <ConsoleState kind="loading" title="正在读取成员">正在确认成员目录与当前权限。</ConsoleState> : <>
      ...
      <section className={styles.memberMetrics} aria-label="成员目录摘要"><article><span>正式成员</span><strong>{query.data.total}</strong><small>当前成员目录 owner 返回</small></article><article><span>管理员</span><strong>{administratorCount ?? "未提供"}</strong><small>{administratorCount === null ? "目录分页未覆盖全体成员" : "来自完整成员目录的管理员角色"}</small></article><article><span>邀请中</span><strong>未提供</strong><small>当前 owner 未返回邀请状态</small></article><article><span>已停用</span><strong>{inactiveCount ?? "未提供"}</strong><small>{inactiveCount === null ? "目录分页未覆盖全体成员" : "来自完整成员目录的停用状态"}</small></article></section>
