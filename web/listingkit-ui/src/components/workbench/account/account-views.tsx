...
export function ProfileView({ data, business, organization, organizationId, section = "summary", identityVerificationOutcomeUnknown = false, onIdentityVerificationOutcomeUnknown, identityContactOutcomeUnknown = false, onIdentityContactOutcomeUnknown }: { data: AccountProfile; business?: AccountBusinessProfile | null; organization?: AccountOrganization | null; organizationId?: string; section?: ProfileSection; identityVerificationOutcomeUnknown?: boolean; onIdentityVerificationOutcomeUnknown?: () => void; identityContactOutcomeUnknown?: boolean; onIdentityContactOutcomeUnknown?: () => void }) {
  ...
  if (section === "settings") return <>{identity}<AccountSettingsPanel data={data} editable returnTo="/workbench/account/profile/settings" identityContactOutcomeUnknown={identityContactOutcomeUnknown} onIdentityContactOutcomeUnknown={onIdentityContactOutcomeUnknown} /><Provenance data={data} /></>;
...
function IdentityContactManagement({ data, returnTo, identityContactOutcomeUnknown, onIdentityContactOutcomeUnknown }: { data: AccountProfile; returnTo: string; identityContactOutcomeUnknown: boolean; onIdentityContactOutcomeUnknown?: () => void }) {
  ...
  return <div className={styles.profileForm}>
    ...
    <form onSubmit={event => { event.preventDefault(); emailMutation.mutate(); }}><label>邮箱地址<input type="email" value={email} onChange={event => setEmail(event.target.value)} disabled={emailMutation.isPending || identityContactOutcomeUnknown} /></label><div className={styles.formActions}><Button type="submit" disabled={emailMutation.isPending || identityContactOutcomeUnknown}>{emailMutation.isPending ? "提交中…" : "修改邮箱"}</Button>{errorText(emailMutation) ? <span className={styles.errorText}>{errorText(emailMutation)}</span> : emailMutation.isSuccess ? <span className={styles.successText}>已发送验证邮件</span> : null}{emailMutation.isError && identityMutationNeedsLogin(emailMutation.error) ? <IdentityLoginRecovery returnTo={returnTo} /> : null}</div></form>
    <form onSubmit={event => { event.preventDefault(); phoneMutation.mutate(); }}><label>手机号码<input type="tel" value={phone} onChange={event => setPhone(event.target.value)} disabled={phoneMutation.isPending || identityContactOutcomeUnknown} placeholder="+8613800000000" /></label><div className={styles.formActions}><Button type="submit" disabled={phoneMutation.isPending || identityContactOutcomeUnknown}>{phoneMutation.isPending ? "提交中…" : "修改手机号"}</Button>{errorText(phoneMutation) ? <span className={styles.errorText}>{errorText(phoneMutation)}</span> : phoneMutation.isSuccess ? <span className={styles.successText}>已发送短信验证码</span> : null}{phoneMutation.isError && identityMutationNeedsLogin(phoneMutation.error) ? <IdentityLoginRecovery returnTo={returnTo} /> : null}</div></form>
...
function IdentityVerificationManagement({ data, returnTo, identityVerificationOutcomeUnknown, onIdentityVerificationOutcomeUnknown }: { data: AccountProfile; returnTo: string; identityVerificationOutcomeUnknown: boolean; onIdentityVerificationOutcomeUnknown?: () => void }) {
  ...
  return <div className={styles.profileForm}>
    ...
    {data.email && data.emailVerified !== true ? <form onSubmit={event => { event.preventDefault(); emailMutation.mutate(); }}><label>邮箱验证码<input value={emailCode} onChange={event => setEmailCode(event.target.value)} inputMode="numeric" disabled={emailMutation.isPending} /></label><div className={styles.formActions}><Button type="submit" disabled={emailMutation.isPending}>验证邮箱</Button><Button type="button" variant="outline" onClick={() => emailResend.mutate()} disabled={emailResend.isPending || identityVerificationOutcomeUnknown}>重新发送</Button>{emailError ? <span className={styles.errorText}>{emailError}</span> : emailMutation.isSuccess || emailResend.isSuccess ? <span className={styles.successText}>{emailMutation.isSuccess ? "邮箱已验证" : "验证码已发送"}</span> : null}{needsEmailLogin ? <IdentityLoginRecovery returnTo={returnTo} /> : null}</div></form> : null}
