...
export function ProfileView({ data, business, organizationId, section = "summary" }: { data: AccountProfile; business?: AccountBusinessProfile | null; organizationId?: string; section?: ProfileSection }) {
  ...
  if (section === "settings") return <>{identity}<AccountSettingsPanel data={data} editable /><Provenance data={data} /></>;
  ...
  if (section === "verification") return <>{identity}<VerificationPanel data={data} interactive /><Provenance data={data} /></>;
  return <>{identity}
...
function AccountSettingsPanel({ data, editable = false }: { data: AccountProfile; editable?: boolean }) {
  return <Panel title="账户设置" description="个人身份资料由 ZITADEL 官方 Auth API 管理"><Fields items={[["显示名称", provided(data.displayName)], ["手机号码", provided(data.phoneNumber)], ["邮箱地址", provided(data.email)], ["所在地区", "未提供"], ["注册时间", "未提供"]]} />{editable ? <><IdentityProfileManagement data={data} /><IdentityContactManagement data={data} /></> : <p className={styles.note}>进入账户设置后，可在 Shuomi 内修改个人资料和联系方式；验证仍由 ZITADEL 官方流程完成。</p>}</Panel>;
}

function IdentityProfileManagement({ data }: { data: AccountProfile }) {
  ...
  return <IdentityProfileForm key={profileQuery.data.userId} data={data} profile={profileQuery.data} />;
}
...
function VerificationPanel({ data, interactive = false }: { data: AccountProfile; interactive?: boolean }) {
  return <Panel title="认证信息" description="认证状态由 ZITADEL 官方 Auth API 返回" className={styles.status}><Fields items={[["手机验证", verification(data.phoneNumberVerified)], ["邮箱验证", verification(data.emailVerified)], ["企业授权", "已读取当前企业授权"]]} />{interactive ? <IdentityVerificationManagement data={data} /> : null}</Panel>;
}

function IdentityContactManagement({ data }: { data: AccountProfile }) {
  const queryClient = useQueryClient();
  ...
  return <div className={styles.profileForm}>
    ...
    <form onSubmit={event => { event.preventDefault(); passwordMutation.mutate({ oldPassword, newPassword }); }}><label>当前密码<input type="password" value={oldPassword} onChange={event => setOldPassword(event.target.value)} autoComplete="current-password" disabled={passwordMutation.isPending} /></label><label>新密码<input type="password" value={newPassword} onChange={event => setNewPassword(event.target.value)} autoComplete="new-password" disabled={passwordMutation.isPending} /></label><div className={styles.formActions}><Button type="submit" disabled={passwordMutation.isPending}>{passwordMutation.isPending ? "提交中…" : "修改密码"}</Button>{errorText(passwordMutation) ? <span className={styles.errorText}>{errorText(passwordMutation)}</span> : passwordMutation.isSuccess ? <span className={styles.successText}>密码已更新</span> : null}</div></form>
