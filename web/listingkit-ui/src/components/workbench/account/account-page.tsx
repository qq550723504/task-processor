...
export function AccountPage({ page, expectedUserId }: { page: PageKind; expectedUserId: string }) {
  ...
  let content;
  if (leaving || authError || identityChanged) content = <ReadError code={identityChanged ? "IDENTITY_CONTEXT_CHANGED" : "AUTHENTICATION_REQUIRED"} />;
  ...
  return <ConsolePage title={title} className={styles.page} breadcrumbs={[{ label: "我的账户" }, { label: title }]} description={page === "profile" ? "查看你的账户信息与当前资料状态" : "查看当前企业信息与项目访问权限"}>{content}</ConsolePage>;
}
...
function AccountRequest({ page, scope, expectedUserId, organizationId, sequence }: { page: PageKind; scope: string; expectedUserId: string; organizationId?: string; sequence: number }) {
  ...
  if (response.isError) return <ReadError code={response.error instanceof AccountReadError ? response.error.code : "UNKNOWN"} />;
  return "effectiveOrganizationId" in response.data ? <OrganizationView data={response.data} /> : <ProfileView data={response.data} />;
}
function ReadError({ code }: { code: string }) {
  ...
  return <ConsoleState kind="error" title={messages[code] ?? "资料读取失败"}>本次未取得资料。请确认登录状态或当前企业后重新读取。</ConsoleState>;
}
