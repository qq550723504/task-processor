...
type PageKind = "overview" | "profile" | "organization";
export function AccountPage({ page, expectedUserId }: { page: PageKind; expectedUserId: string }) {
  ...
  const title = page === "overview" ? "我的账户" : page === "profile" ? "账户资料" : "企业空间";
  ...
  else if (context.isSwitching || ((page === "organization" || page === "overview") && context.isLoading)) content = <ConsoleState kind="loading" title="正在确认当前上下文">旧资料已清除。</ConsoleState>;
  ...
  return <AccountShell pathname={page === "overview" ? "/workbench/account" : `/workbench/account/${page}`} title={title} description={page === "overview" ? "查看当前账户、企业与收益状态" : page === "profile" ? "查看你的账户信息与当前资料状态" : "查看当前企业信息与项目访问权限"}>{content}</AccountShell>;
}
...
function ReadError({ code, page = "profile" }: { code: string; page?: PageKind }) {
  ...
  const returnTo = page === "overview" ? "/workbench/account" : `/workbench/account/${page}`;
