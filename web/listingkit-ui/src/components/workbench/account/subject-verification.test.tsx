...
it("submits once and keeps uncertain outcomes blocked while allowing status refresh",async()=>{
 const fetch=vi.fn().mockImplementation((_url:string,init?:RequestInit)=>Promise.resolve(init?.method==="POST"?Response.json({code:"RESULT_UNVERIFIED"},{status:502}):Response.json(initial)));vi.stubGlobal("fetch",fetch);mount();
...
it("shows verified enterprise facts without elevating personal or platform authorization",async()=>{
 vi.stubGlobal("fetch",vi.fn().mockResolvedValue(Response.json({...initial,state:"VERIFIED",canStart:false,applicationId:"app",companyName:"测试企业",creditCode:"91310000MA00000001",verifiedAt:"2026-09-26T00:00:00Z",isApplicant:true})));mount();
 expect(await screen.findByText("企业认证已通过")).toBeVisible();expect(screen.getByText(/认证不会增加平台权限/)).toBeVisible();
 await userEvent.click(screen.getByRole("button",{name:"个人认证"}));expect(screen.getByText("个人认证暂不可用")).toBeVisible();
});
