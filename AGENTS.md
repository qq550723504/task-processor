...
上述条件首先作为**风险提示线**，不是机械拆 PR 的目标。命中后先识别本次真正新增的高风险边界：如果任务本身仍是一个完整用户结果、边界已有合同且可以在同一 Delivery Batch 内安全推进，可以保持一个主要 PR，并在高风险边界设置一次里程碑检查；只有存在可独立交付的用户结果、独立生命周期/风险，或确实无法在同一批次安全评审时才拆分。若出现尚未设计的真实高风险边界，再先完成相应架构设计和独立评审。
