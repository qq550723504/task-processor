...
- `recordId`：owner 在组织 200 的实际 POST 记录；`organization300RecordId`：同一 owner 在组织 300 的实际 POST 记录；`readonlyRecordId`：先 POST 再在外部 grant 替身撤销写权限的记录。
...
- 组织 `200`（Fixture A）和 `300`（Fixture C）都由实际 Catalog Publisher + Listing POST 建立资料，验证从 200 不可见 300 的记录、切到 300 后可见；`100`（Fixture B）保留为空组织，覆盖合法空集与 cursor 跨组织失败。既有 OrganizationSwitcher 仍通过实际 BFF/context PUT 切换。
...
自动模式验证列表两页、两个非空组织、组织admin/operator/read-only/Store-only、合法空组织、cursor跨组织失败，并使用列表实际返回ID调用诊断；它还直连 Go 断言成功、缺身份、无权限、撤销、依赖失败和非法输入都带 `no-store`/`nosniff`。同时保留诊断两 action、expected digest、一致性重读、非 owner/跨组织隔离、撤销、Go超时、组织断言、缺session、输入和method边界、真实context、public session不暴露token。正常 Go suite 仅跳过这个显式启动 fixture；自动模式额外对同一隔离 PostgreSQL 执行相关 Go 回归，其测试使用各自随机 schema。
