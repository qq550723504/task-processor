...
- Route only the secondary BFF's business calls through a run-owned loopback observer. The observer never drops a request merely because a test mode is armed. The same held path first receives a live healthy request, is explicitly released, reaches Go exactly once, and returns 200. For post-body cancellation and deadline controls, require the real BFF-to-observer connection to close before explicit release; releasing afterward must complete the observer handler with zero Go calls. Separately stream a partial browser request body, cancel it while the BFF is reading, wait past the 15-second total deadline, and require that it never reaches the observer or Go. Record BFF observer receipts, Go dispatches, real client-connection closes, explicit releases, handler completion, and open handlers as distinct observations. Already dispatched operations retain their original idempotency/receipt/unknown-outcome rules; these zero-dispatch controls do not claim rollback.
