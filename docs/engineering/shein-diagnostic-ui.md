...
- 企业上下文错误先使用既有 provider 重载；只有成功且身份、企业、角色仍一致时才续读诊断。恢复期间按钮禁用，切动作/企业或卸载后的迟到恢复不触发旧请求。provider 的 retry 返回读取结果，现有不消费返回值的调用方行为不变。
- 页面仅消费同源只读 client。无发布、Apply、自动生成、报告持久化、轮询或假数据 fallback。

## 产品体验入口

从现有 AI工作台 → 任务中心 → 已完成进入本地资料准备工作记录，选择实际返回的一项，在右侧结果点击“查看诊断”，再用“返回任务中心”返回并重新读取当前范围。无需复制记录ID或拼详情URL；旧 SHEIN 首页卡片与独立列表入口保持撤回。启动隔离环境、headed/debug操作和停止步骤见 [任务中心资料准备记录](shein-records-ui.md)。

## 诊断专用回归（开发验证）

下面保留单独诊断测试的运行方法；它不代替从工作台发现资料的产品验收。
