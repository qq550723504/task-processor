...
- `cmd/`
  ...
  - `commercial-owner-schema-migrate` 由 `scripts/commercial-owner-schema-migrate.ps1` 维护；仅针对私有 manifest 指定的独立 commercial-owner 数据库初始化当前 money、commercial billing 与 orgresource schema，不自动切换数据库或操作生产数据。
