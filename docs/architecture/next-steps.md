...
- `TestHTTPAPIModulesFileDoesNotOwnFeatureBuildWrappers` 禁止 `internal/app/httpapi/modules.go` 吞入 feature build wrapper，保持 feature build wrapper 留在专用 builder seam
- `TestHTTPAPIModulesFileDoesNotOwnBootstrapOrchestration` 禁止 `internal/app/httpapi/modules.go` 吞入 bootstrap orchestration，保持 bootstrap 编排留在专用 orchestration seam
- `TestHTTPAPIModulesFileDoesNotOwnLegacyBuildHandlersFacade` 禁止 `internal/app/httpapi/modules.go` 吞入 legacy build handlers facade，保持历史 build facade 约束在专用兼容 seam
...
- `TestHTTPAPIRuntimeDepsMethodsDoNotOwnFeatureHTTPAPIModuleTypes` 禁止 `internal/app/httpapi/runtime_deps_methods.go` 承接 feature HTTP API module 类型，保持 feature module 类型只留在专用 module/builder seam
