...
| 1688 ListingKit task creation | `internal/compatibility/listingkit/sourcehandoff/a1688/httpapi/handler.go` (`CreateListingKitTask`) and its registered route in `internal/compatibility/listingkit/sourcehandoff/a1688/httpapi/routes.go` | Reserve under the authenticated tenant and stable source/task identity; commit only after the defined successful billable outcome, never merely on route acceptance. |
