## Task Processor Framework Phase 28 Scope Recommendation

### Candidate Directions

`Phase 27` 收口之后，ListingKit execute/action 邻域里至少还有两个可继续推进的方向：

#### 方向一：ListingKit action execute handoff branch-result routing ownership
...
- `fromRetryPage / fromQueuePage` result routing
...
`Phase 28` 应先聚焦 **ListingKit action execute handoff branch-result routing ownership**。
...
#### 1. `Phase 27` 已把 handoff seam 里的 branch-local invocation 收干净
...
- invocation 责任已经从 handoff seam 中被剥离
- 再围着 invocation 本身做对称性打磨，收益已经下降

#### 2. 下一块真实压力已经转移到 handoff seam 自己的 branch-result routing 并置
...
这说明根因不再是“谁拥有 branch-local invocation”，而是：

- handoff seam 自己还在同时持有 branch orchestration 与 result routing
...
相比之下，handoff seam 本地 branch-result routing：

- 影响范围更集中
- 行为语义更清晰
- 可以先在 handoff seam 局部收一刀，而不碰别的 consumer

#### 4. 这条线还能顺着 `Phase 26/27` 刚建立的 seam 继续推进
...
- result-adaptation seam

所以 handoff seam 自己的 branch-result routing 再切一小刀，会和前面几轮的 ownership 收口风格非常一致，不需要引入新的抽象风格。

### Why Not Reopen Shared Queue / Retry Clone Helpers First

不建议 `Phase 28` 继续优先追 shared `queue/retry` clone helper ownership，原因有三个：
...
#### 3. handoff seam 自己的 branch-result routing 混合职责更像眼前的根因

既然 handoff seam 已经明确同时持有 branch selection、branch-local phase 调用和 result routing，就没必要跳过这个更近、更具体的 residual hotspot，直接去碰更宽的 shared helper 问题。

### Proposed Phase Shape

一个合适的 `Phase 28` 形状应该是：

1. 保持 `Phase 27` 的 handoff / branch-local invocation split 不动
2. 不重开 shared `queue/retry` helper 的 broader multi-consumer owner
3. 只围绕 handoff seam 的 branch-result routing 下刀
4. 先明确哪些 result routing 可以封装成更明确的 local branch-result home
5. 把 handoff seam truly local branch selection 留在 handoff home
6. 用 focused tests 和 ownership guardrail 锁住“harness 更清晰、outward behavior 不变”的方向

### Guardrails

`Phase 28` 建议保持以下 guardrails：

1. 不要回头重开 `Phase 27` 已稳定的 handoff / branch-local invocation split
2. 不要把范围扩成 shared `queue/retry` helper 的全面大改
3. 不要改变 action execute outward behavior
4. 不要引入新的 generic routing framework
5. 优先把 handoff seam 自己的 local branch-result routing 收清楚

### Recommended Next Step

下一步建议直接写一个 `Phase 28` implementation plan，主题明确限定为：

**ListingKit action execute handoff branch-result routing ownership**
...
- `Phase 28` 应先聚焦 handoff seam 本地的 branch-result routing ownership
