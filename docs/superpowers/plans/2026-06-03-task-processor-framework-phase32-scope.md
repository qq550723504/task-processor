## Task Processor Framework Phase 32 Scope Recommendation

### Candidate Directions

`Phase 31` 收口之后，ListingKit execute/action 邻域里至少还有两个可继续推进的方向：

#### 方向一：ListingKit action execute handoff result normalization ownership
...
也就是 unified handoff result 邻域仍然保留：
...
`Phase 32` 应先聚焦 **ListingKit action execute handoff result normalization ownership**。
...
#### 1. `Phase 31` 已把 mode-routing pairing 收干净
...
- mode-routing pairing 责任已经从 broad routing body 中被剥离
- 再围着 mode pairing 本身做对称性打磨，收益已经下降

#### 2. 下一块真实压力已经转移到 unified handoff result 的双轨镜像职责

当前 result layer 里最明显的 mixed concern 是：

- `fromRetryPage / fromQueuePage`
- `persistenceQueueFromRetryPage / persistenceQueueFromQueuePage`

这说明根因不再是“谁拥有 mode pairing”，而是：

- 为什么 unified handoff result owner 仍然要沿 retry/queue 两条镜像路径同步演进
...
相比之下，unified handoff result normalization：

- 影响范围更集中
- 行为语义更清晰
- 可以先在 result layer 局部收一刀，而不碰别的 consumer

#### 4. 这条线还能顺着 `Phase 30/31` 刚建立的 seams 继续推进
...
所以下一刀继续收 unified result normalization，会和前面几轮的 ownership 收口风格非常一致，不需要引入新的抽象风格。

### Why Not Reopen Shared Queue / Retry Clone Helpers First

不建议 `Phase 32` 继续优先追 shared `queue/retry` clone helper ownership，原因有三个：
...
#### 3. unified result normalization 混合职责更像眼前的根因

既然 unified handoff result 邻域已经明确同时保留 retry/queue 两条镜像路径，就没必要跳过这个更近、更具体的 residual hotspot，直接去碰更宽的 shared helper 问题。

### Proposed Phase Shape

一个合适的 `Phase 32` 形状应该是：

1. 保持 `Phase 31` 的 mode-routing / mode-pairing split 不动
2. 不重开 shared `queue/retry` helper 的 broader multi-consumer owner
3. 只围绕 unified handoff result normalization 下刀
4. 先明确哪些 retry/queue 镜像职责可以封装成更明确的 local normalization home
5. 把 outward retry/queue behavior 保持不变
6. 用 focused tests 和 ownership guardrail 锁住“harness 更清晰、outward behavior 不变”的方向

### Guardrails

`Phase 32` 建议保持以下 guardrails：

1. 不要回头重开 `Phase 31` 已稳定的 mode-routing / pairing split
2. 不要把范围扩成 shared `queue/retry` helper 的全面大改
3. 不要改变 action execute outward behavior
4. 不要引入新的 generic normalization framework
5. 优先把 unified handoff result 自己的 local normalization ownership 收清楚

### Recommended Next Step

下一步建议直接写一个 `Phase 32` implementation plan，主题明确限定为：

**ListingKit action execute handoff result normalization ownership**
...
- `Phase 32` 应先聚焦 unified result normalization ownership
- 不建议现在就全面重开 shared `queue/retry` clone helper ownership
- 不建议把这一刀扩大成 generic normalization abstraction
