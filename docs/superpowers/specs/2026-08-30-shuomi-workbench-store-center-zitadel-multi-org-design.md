...
- 浏览器发起企业作用域请求时携带启动时的 expected Organization 一致性断言。BFF 仅将其与 HttpOnly 选择 cookie 比较；不一致时 fail closed，且该值不得用于选择企业、替代 cookie、跳过授权或转发为新的身份来源。这样跨标签页切换也不能把旧请求重新解释到新企业。
...
- 创建/删除的显式重试必须复用启动时的 Organization 与幂等键；新的用户提交才生成新键；
