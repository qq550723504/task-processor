...
| 选项 | 代价 | 建议 |
|---|---|---|
| **(a) 接受 + UI 显式展示剩余额度** | 零契约变更 | 先做（不静默失败，且相对 ③ 不是回归） |
| (b) 提高 256 常量 | 一行 + 同步修订 `src2b-public-acquisition.md:32` 的 limits 段 | **建议一并做**：否则批量终身上限 = 25 批 |
| (c) 为终态行加保留期 / `DELETE` | **推翻**已记录的 "no TTL/key GC" ⇒ 需独立评审 | backlog，由真实使用触发 |
...
⇒ 在 256 终身额度**确认存在**的前提下，备选形状在额度语义上更正确（额度 = 成功采集的商品数，与 ③ 完全一致），且**不碰任何采集契约**。取舍见 §10-2。
...
2. **形状取舍**（§8.3）：继续「提交时预建 `acquiring` 行」（进度可见，但无效链接永久消耗额度），还是改用「执行器持本地队列 + 抓完才 `StartPrepared`」（不消耗无效额度、不碰采集契约，但提交后看不到待办）？
3. **是否一并把 256 提高**（§8.1 选项 b）？否则批量终身上限 = 25 批，且**当前已上线的 ③ 同样受此限**。
