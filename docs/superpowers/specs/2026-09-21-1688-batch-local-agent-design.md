...
> **局部 SUPERSEDED（2026-09-26）**：本设计的「不自动识别或绕过验证码」与「不恢复 ①（服务端匿名裸 HTTP）」两条，已被 [`PD-1688-SERVER-PUBLIC-BROWSER-2026-09-26`](../../product/pd-1688-server-public-browser-2026-09-26.md)（Issue #514）**逐条推翻**：服务端匿名浏览器采集与自动验证码处理已由产品决定允许。**本设计其余内容（路线 B 本地执行器、批量队列、scope 固定、崩溃安全等）继续有效——这不是取消本设计。**
...
2. **遇到验证码时**：整批暂停并提示，由用户处理后继续；**执行器不尝试识别或绕过验证码**（§4 D1.3）。
   > **范围说明**：`PD-1688-SERVER-PUBLIC-BROWSER-2026-09-26` 推翻的是**服务端**匿名采集的同类限制；**本条描述本地执行器职责，继续有效**。
...
- 不恢复 ①（服务端匿名裸 HTTP）—— 已证结构性不可行
  > **`SUPERSEDED`（2026-09-26，`PD-1688-SERVER-PUBLIC-BROWSER-2026-09-26`）**：「裸 HTTP 结构性不可行」这一事实仍成立（真实页被 `punish` 拦截），但**不得据此禁止服务端匿名采集**——该采集改为浏览器 provider（Issue #514）。
- 不引入后台调度器、TTL 或 key GC（§1.5 第 2 条）
- **不自动识别或绕过验证码**（遇到验证码即整批暂停、交用户处理，§4 D1.3）
  > **`SUPERSEDED`（2026-09-26，`PD-1688-SERVER-PUBLIC-BROWSER-2026-09-26`）**：作为**产品级禁止**已被推翻（用户已知悉并接受站点条款/风控/自动处理成功率风险）。对**本地执行器**仍是推荐做法，见 §1.2 第 2 条。
...
| # | 护栏 | 具体做法 | 不这么做会怎样 |
|---|---|---|---|
| **1** | **scope 固定（最关键）** | 批量启动时记录当时的 `actor` + Effective Organization 作为**本批批准 scope**（§4 D1.4 要求这个值由**用户显式确认**后才成立）；每条点击**之前**重新读取页面当前展示的 scope，必须与批准 scope 逐字节一致；不一致 ⇒ **立刻停止整批、不提交**并提示用户。**而且「读 scope」与「点按钮」必须在同一个不可中断的浏览器任务里完成**（见下） | 应用在点击时是按**页面当前**上下文提交的（`consent` 取自当前 scope），它并不知道本批是从哪个身份启动的；只有执行器知道 ⇒ 可能把商品归属到用户未批准的组织/账号 |
...
**护栏 1 的竞态：读写必须原子（第十一轮评审 #26）。** 仅靠「先读 DOM 再点按钮」**不够**。`CaptureReceiver.run()` 的快照取自**点击处理函数执行的那一刻**的页面 React state，而应用的 BFF 校验是拿**这个快照自己跟自己比**（`capture-receiver.tsx:70-79`/`110-119`）。所以存在这个窗口：

```
执行器读 DOM → 读到 A
        ↓（此时另一个标签把企业切到 B）
执行器点按钮 → 处理函数快照到 B → BFF 校 B 对 B → POST 被允许到 B
```

⇒ 要求：**「读 scope」与「点按钮」必须在同一个不可中断的浏览器任务内完成**（单次 `Runtime.evaluate` 同步函数：先读 DOM、比较、比较通过后立即 `.click()`，中间**不得有 await / 不得分两次求值**）。这样处理函数的快照不可能与刚读到的值不同（React 状态变更必须跨任务边界）。

**这件事执行器做不到、必须如实记录的部分（未决，我不自行决定）。** 即使做到了原子点击，还有一个**服务端侧**的窗口：应用是按其**活会话**上下文提交的（`OrganizationAccessPolicyLiveWrite`，`browser_capture_application.go:113`，在请求时才读活组织）。如果活会话在「点击」与「BFF 读活组织」之间变了，POST 仍会落到 B。

- 执行器**无法**关闭这个窗口——只有服务端能把「期望组织」绑定进请求/护栏，而那是**改采集契约**（§1.3 明确不做，且属我的 §17 第 2/3 条保留给 owner 的事）。
- **可做的局部收敛**（已写入执行模型）：专用 profile 只跑执行器自己开的标签、禁止并行执行器（§4 D3）、发现身份/标签异常即暂停。代价是用户仍可在同一 profile 的另一个标签里手动切企业。
- ⇒ 结论：**此残留项需产品决定**（要么加服务端「期望组织」绑定 = 契约变更，要么显式接受该残留）。它是 **S2 的阻塞项，不是 S1 的**——S1 只走 fixture，不会向真实企业提交。
...
- **检测由执行器自己做，不依赖扩展的返回码**：`extensions/1688-capture/src/background.ts:39` 把所有采集失败塔缩为 `ok:false, code:'ACTION_UNAVAILABLE'`，执行器**无法**从扩展响应里分辨原因。所以分类必须来自执行器**自己的页面观测**（它在导航后、触发采集前直接读页面）。
- **两个独立判定，不得混成一个「正向确认」**（第十轮评审 #24）：

  | 判定 | 观测对象 | 结论 |
  |---|---|---|
  | **A. 挑战/风控判定** | 是否命中挑战/登录/校验页（URL 跳转 + 页面标记） | 命中 ⇒ **整批暂停**（本节的规则） |
  | **B. 商品数据判定** | 是否取到设计要求的字段 | 取到 ⇒ 正常采集；未取到且 **A 未命中** ⇒ 按 §7「单条失败」处理并**继续下一条** |

- **为何必须分开**：下架/无效商品的页面同样没有商品 DOM。若把「取不到就暂停」当成统一规则，**一个坏链接就会停掉整批**，违背保序的部分失败行为（§7）。
- **剩余歧义的定义行为**：若 A 未命中、B 未取到，且页面**也没有给出确定的「已下架/不存在」标记** ⇒ 默认**暂停并交人**。代价是用户多看一眼；反向默认可能把一次风控静默当成下架而跳过一个商品。这是**有意的取舍**，不是遗漏。
- **已知信号**（来自本仓库实测，非推测）：匿名裸请求命中风控时返回 HTTP 200 但带 `Bxpunish`/`x5secdata` 且页面不含商品数据；浏览器侧表现为跳到登录/校验页而取不到商品 DOM。
- **精确签名不在本设计里臆造，也不能靠「一次真实命中」来定**（第十一轮评审 #27）：§17 第 8 条已明确 S1 只跑 fixture、真实风控检查为 `NOT_RUN`，所以「等到 S1 遇到一次真实验证码再固定签名」是**自相矛盾**的。⇒ 改为定义一个**可观测的契约**，用**脱敏 fixture** 验收：

  | 信号 | 来源 | 判定 |
  |---|---|---|
  | URL 最终落到登录/校验类页面 | 执行器自己读 `location` | A 命中 ⇒ 暂停 |
  | 页面命中风控标记（本仓库实测过的 `Bxpunish` / `x5secdata`） | 执行器自己读页面 | A 命中 ⇒ 暂停 |
  | `200` 但**既无商品 DOM 也无风控标记** | 执行器自己读页面 | A 未命中、B 未取到、无确定下架标记 ⇒ 默认**暂停交人** |
  | 有确定的下架/不存在标记 | 执行器自己读页面 | 归 §7「单条失败」并继续 |

  验收用**本地脱敏 fixture**（构造出上述四种页面，不含任何账号数据、不发真实请求）。风控标记的证据来自本仓库此前对匿名请求的实测（``200 + Bxpunish/x5secdata + 无商品数据``），**不是猜测的选择器**。真实页面上的确认是**另一个需用户单独授权的验收**，本文档保留 `NOT_RUN`。
...
#### D1.4 批量启动必须先**展示并绑定** scope（一次确认）

**这条是第八轮与第九轮共同指向的缺口，且是 BLOCKER 级**：§1.6/D1.2 说「批量启动时记录当时的 actor + Effective Organization 作为本批批准 scope」，但上一版**从未定义**批量启动时用户在哪里**看到**这个 scope 并**做出确认**——它只是读了当前会话状态。

后果很具体：**如果已持久化的 profile 恰好登录在另一个企业下，那么事后每一条的相等校验都会通过**，执行器就会整批自动提交到那个企业。**观测到的会话状态不是 consent。**

⇒ 修正为明确的**三步**（在任何采集/交付/提交**之前**）：

| 步 | 动作 |
|---|---|
| 1 | 执行器**从应用读取服务端已核实的身份**（`capture-receiver.tsx:139-140` 展示的那个值），而不是从浏览器 profile / cookie 自行推断 |
| 2 | **把这个 actor + Effective Organization 展示给用户并要求显式确认**（CLI 提问 / 本地交互；S1 即以此作为批启动的入口）。若用户回答「不对」⇒ **不启动** |
| 3 | 把确认后的 scope **写入本地队列（崩溃安全，§4 D2.2）**，作为本批**唯一权威**的批准 scope |

⇒ 之后每条提交前的相等校验（护栏 1）都是与**这个已确认并已落盘的值**比，而不是与「当前会话恰好是什么」比。

**fail-closed**：重启后队列中没有「已确认 scope」记录，或其不可读/不可信 ⇒ **停止整批**（绝不自动把当前会话当成批准 scope）。这与 §4 D2.2 第 3/4 条同构。

> **取用点说明**：应用展示已核实身份是在采集页上。因此批启动的确认发生在**第 1 条商品页交付后、提交之前**——这也正好是 S1（单条驱动）能独立验证该步骤的地方，不需要新增任何路由或页面。验收见 §10「批启动 scope 确认」。
...
- **区分「回退为可重采」与「记录终态结果」**（第十轮评审 #25）：上面的两个条件只用于**把 `submitting` 退回 `captured`（即可重采）**。若 scope 一致的 by-key 回读拿到 **`200` 终态**（`published`/`failed`），那是**确定性结果**，应当**直接记录**（与 §4 D2 分支表一致），不是 `outcome_unknown`——不要因为「曾处于 `submitting`」而丢弃一份确定的发布回执。
...
| 场景 | 结果 |
|---|---|
| 单条失败（下架 / 无效链接） | 记录本地失败并**继续下一条**；不自动重试该条 |
| **挑战页 / 验证码 / 风控页** | **不得当作单条失败继续**。按 §4 D1.3 处理：**整批暂停**，用户处理完后**原条目重做**（重导航 → 重采集）。⇒ 本行必须与上面的「单条失败」**分行**（第九轮评审 #22） |
...
| 执行器进程被杀 | 重启读回本地队列；先校 scope：不一致 ⇒ 整批停止；一致 ⇒ **按 §4 D2 分支表处理**。⇒ `submitting` 且 scope 一致的 `200` **终态**（`published`/`failed`）**必须记下该终态结果**；只有**读不到确定性结果**（`200` 非终态 / `404` / scope 不一致 / 队列不可读）才记 `outcome_unknown`（第十轮评审 #25）|
...
| **批启动 scope 确认（§4 D1.4）** | 让 profile 登录在**另一个**企业下启动批量 ⇒ 执行器在第 1 条交付后展示的正是那个企业，用户**不确认**即整批不启动、**无任何 POST**（验证「观测到的会话状态不会被当作 consent」） |
| **无已确认 scope 则不启动** | 删掉本地队列里的已确认 scope 后重启 ⇒ 整批停止且无新 POST（**不**自动采用当前会话） |
| **读 scope 与点击原子（§4 D1.2 护栏 1）** | fixture 中在「读 DOM」与「点击」之间注入一次 React 状态变更 ⇒ 执行器的单次同步 `Runtime.evaluate` **不可能**让两者不一致；代码审查确认该路径**无 await、无分次求值** |
| **挑战判定来自执行器自己的观测（§4 D1.3）** | 用 4 个**脱敏本地 fixture**（登录跳转页 / 风控标记页 / `200` 无商品无标记页 / 确定下架页）驱动分类器 ⇒ 前两者暂停整批、第三者默认暂停、末者记单条失败并继续 |
...
8. **【新增，待决】服务端侧的「期望组织」绑定（第十一轮评审 #26）**：即使执行器做到原子「读 scope + 点击」，应用仍按**活会话**提交（`OrganizationAccessPolicyLiveWrite`）。若活会话在点击与 BFF 读活组织之间变了，POST 可落到另一个组织。关闭它需要服务端把「期望组织」绑定进请求/护栏，那是**改采集契约**（§1.3 不做）。二选一：
   - (a) 加服务端期望组织绑定（契约变更，需 owner 与产品决定）；
   - (b) **显式接受该残留**，并用局部收敛（专用 profile、单执行器、发现身份异常即停）把可利用面压到「用户自己在同一 profile 另一个标签里手动切企业」。
   > 建议 (b)。它是**人为的本地破坏**，不是远程/跨租户攻击面；且命中它需要用户在批量运行的同时在同一 profile 手动切企业。**此条阻塞 S2 真实提交，不阻塞 S1。**
...
### 15.9 第九轮 2 条（针对 v3.5）

| # | 评审内容 | 分类 | 处置 |
|---|---|---|---|
| 22 | **必须显式批准批量 scope** | **BLOCKER（成立，已修）** | 核实成立，而且是 consent 层的真缺口：我写了「批量启动时记录当时的 actor + Effective Organization 作为批准 scope」，但**从未定义用户在哪里看到它并做出确认**——只是读了当前会话状态。⇒ 若 profile 恰好登录在**另一个**企业，事后每条相等校验都通过，执行器会整批提交到那个企业。**观测到的会话状态不是 consent。已新增 §4 D1.4**：三步（从**应用**读服务端已核实身份 → 展示并要用户显式确认 → 崩溃安全落盘为本批唯一权威 scope）；无已确认 scope 或不可读 ⇒ **停止整批**（fail-closed），绝不自动采用当前会话。§10 新增两项验收 |
| 23 | **挑战页应暂停而非终结** | **IMPLEMENTATION_TEST（成立，已修）** | 核实成立，而且与 §4 D1.3 直接矛盾：§7 原来把「挑战页」与「下架/无效链接」写在**同一行**（「单条失败 ⇒ 继续下一条」），而 D1.3 要求整批暂停、用户处理后重做该条。已把**挑战页/验证码/风控页分行**，明确归 §4 D1.3（整批暂停 + 原条目重做），并注明必须与「单条失败」分行 |

第 22 条又是 consent 相关（与 #20 同类，但更根本：那条是我漏了契约文本，这条是我把「读取当前状态」当成了「取得同意」）。第 23 条是一处**表格自相矛盾**——同一文档两处规则不一致，属实现前必须消除的模糊性。

### 15.10 第十轮 2 条（针对 v3.6，均 IMPLEMENTATION_TEST）

| # | 评审内容 | 分类 | 处置 |
|---|---|---|---|
| 24 | **下架商品与挑战页必须可区分** | **IMPLEMENTATION_TEST（成立，已修）** | 成立：§4 D1.3 原来要求「不能正向确认就暂停」，而 §7 要求下架/无效商品（同样没有商品 DOM）归「单条失败并继续」⇒ 两者矛盾，**一个坏链接会停掉整批**。另：`background.ts:39` 把所有采集失败塔缩为 `ACTION_UNAVAILABLE`，执行器无法从扩展响应分辨原因。**已重写 §4 D1.3**：(a) 明确分类必须来自执行器**自己的页面观测**，不依赖扩展返回码；(b) 拆成**两个独立判定**——A 挑战/风控判定（命中 ⇒ 整批暂停）、B 商品数据判定（未取到且 A 未命中 ⇒ 单条失败并继续）；(c) 定义剩余歧义：A 未命中 + B 未取到 + 无确定「已下架」标记 ⇒ 默认暂停交人（有意取舍，说明理由） |
| 25 | **重启恢复不得丢弃确定性终态** | **IMPLEMENTATION_TEST（成立，已修）** | 成立：§7 原写「凡属 `submitting` 的条目一律 `outcome_unknown`」，会**压过** §4 D2 分支表——即使 scope 一致的 by-key 回读已拿到 `200` `published`/`failed`（确定性结果），也会被丢掉并报一个不必要的 unknown。**已修**：§7 改为「先按 §4 D2 分支表处理，`200` 终态必须记下该结果；只有读不到确定性结果（非终态 / `404` / scope 不一致 / 不可读）才记 `outcome_unknown`」；§4 D2.1 新增一句明确区分「回退为可重采」与「记录终态结果」 |

两条都是我自己造成的**文档内部矛盾**，不是新概念：一条是两个判定混成了一个「正向确认」，一条是一条生硬的 blanket 规则压过了更细的分支表。

### 15.11 第十一轮 2 条（针对 v3.7）

| # | 评审内容 | 分类 | 处置 |
|---|---|---|---|
| 26 | **把批准 scope 原子地绑定到点击** | **BLOCKER（成立，已部分修 + 残留转产品决定）** | 成立且很锐利：即使有了已落盘的批准 scope，执行器侧的 DOM 比较与点击**不是原子的**。`CaptureReceiver.run()` 在点击处理函数执行时才快照页面 scope，BFF 拿**这个快照自己跟自己比** ⇒ 若在「执行器读到 A」与「点击处理函数执行」之间企业变成 B，处理函数快照到 B、BFF 校 B==B、POST 被允许到 B。**已修（执行器侧）**：§4 D1.2 护栏 1 要求「读 scope」与「点按钮」在**同一个不可中断的浏览器任务**内完成（单次同步 `Runtime.evaluate`，中间不得有 await / 不得分次），§10 新增对应验收。**残留（我不自行决定）**：应用按**活会话**提交（`OrganizationAccessPolicyLiveWrite`），若活会话在点击与 BFF 读活组织之间变了，POST 仍可落 B；关闭它需改采集契约。⇒ 已录为 **§11-8**，建议 (b) 显式接受 + 局部收敛；**阻塞 S2，不阻塞 S1** |
| 27 | **挑战信号不能靠一次被禁止的真实运行获得** | **IMPLEMENTATION_TEST（成立，已修）** | 成立，又是我的**自相矛盾**：D1.3 说「精确签名留到 S1 用一次真实命中固定」，而 §17 第 8 条又禁止真实运行并定 `NOT_RUN`。**已修**：§4 D1.3 改为定义一个**可观测契约** + **四个脱敏本地 fixture**（登录跳转 / 风控标记 / `200` 无商品无标记 / 确定下架），风控标记的证据来自本仓库此前对匿名请求的**实测**（`200 + Bxpunish/x5secdata + 无商品数据`）而非猜测；§10 新增对应验收；真实页面确认仍为单独授权项、保持 `NOT_RUN` |

第 26 条特别值得记：它证明我前几轮把「护栏」当成了安全证明，实际上「读一个值」与「根据它行动」之间如果不原子，护栏本身就有 TOCTOU。这比前几轮的错误更深一层。

## 15.12 设计冻结声明（第十一轮之后）

按 AGENTS「架构评审停止规则」，本设计**到此冻结并视为 `IMPLEMENTATION_READY`**：

- 已过 **十一轮**评审、共 **27 条** finding，全部核实；其中 **BLOCKER 类** 均已修或已明确转为产品决定（§11-8）。
- 第十一轮之后，**非 BLOCKER 的 finding 一律转为 `IMPLEMENTATION_TEST` / `BACKLOG`，不再为它们增加设计轮次**——继续对轮只会让修正本身产生新的文字矛盾（第八至十一轮已反复出现这个模式）。
- **冻结节制下唯一可以重新打开设计的条件**：出现新的 **BLOCKER**（命中 AGENTS 的 BLOCKER 条件之一）。
- ⇒ 下一步是**编码**（§14 S1），不再是更多文档。
...
15. **新增 §4 D1.4（批启动 scope 确认）**：第九轮评审 #22 指出我把「观测到的会话状态」当成 consent——profile 若登录在另一企业，事后校验全通过，会整批提交错归属。已改为「从应用读已核实身份 → 展示并要用户确认 → 崩溃安全落盘 → 后续只与该值比」，无已确认 scope 则不启动。
16. **§7 挑战页分行**：第九轮评审 #23 指出 §7 把挑战页归入「单条失败⇒继续」与 §4 D1.3 直接矛盾。已分行并归入 D1.3。
17. **§4 D1.3 拆为两个独立判定**：第十轮评审 #24 指出「不能正向确认就暂停」会让一个下架链接停掉整批。已拆为 A（挑战/风控 ⇒ 暂停）与 B（商品数据 ⇒ 单条失败），并明确分类来自执行器自己的页面观测（`background.ts:39` 把失败塔缩为 `ACTION_UNAVAILABLE`，不能依赖扩展返回码）；剩余歧义默认暂停交人。
18. **重启恢复保留终态**：第十轮评审 #25 指出 §7 的 `submitting ⇒ 一律 outcome_unknown` 会丢掉 scope 一致的 `200` 终态回执。已改为先走 §4 D2 分支表，只有读不到确定性结果才归 `outcome_unknown`。
19. **护栏 1 的 TOCTOU**：第十一轮评审 #26 指出「读 scope 再点按钮」之间存在竞态，处理函数自己的快照会让 BFF 自比自通过。已要求读写在同一不可中断任务内完成；服务端侧残留转为 §11-8。
20. **挑战判定改为可观测契约 + 脱敏 fixture**：第十一轮评审 #27 指出「用一次真实命中固定签名」与 §17 禁止真实运行自相矛盾。已改为定义可观测契约并用四个脱敏 fixture 验收。
21. **新增 §15.12 设计冻结声明**：按 AGENTS 停止规则冻结，非 BLOCKER 不再开设计轮次。
...
- **证据来源**：fixture 页面 + 假扩展（覆盖 §10 的确定性项：单条等价性、scope 固定与原子、reset、预写落盘、损坏队列、挑战分类四项 fixture）。**§10 中依赖真实 1688 风控/登录态的项保留 `NOT_RUN`**。
