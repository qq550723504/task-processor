...
**实现方式**：执行器**驱动 popup 页面模拟真实点击**，而不是直接发 `popup.new` 消息。理由：`background.ts:28` 只接受来源为 `popup.html` 且无 `tab` 的消息，所以两种方式都必须驱动 popup；模拟点击不依赖内部消息字符串契约。

**同时如实说明它做不到什么**（第八轮评审 #20 的教训）：驱动 UI **不能**把一个动作变成「用户的选择」。扩展的消息准入（`background.ts:28`）只检查来源页面与 `tab`，**无法区分**模拟点击与真实点击。因此「由用户自己做出选择」这一层是**契约层要求，不是被代码强制执行的边界**；批量场景下它由 §1.6 的一次性用户决定 + §12 的文档待同步项承担，而不是靠模拟点击冒充。本设计**不**为此新增扩展侧校验（那会修改 ③ 的行为，超出范围）。同样适用于 `popup.capture`（「采集当前商品」）与 `popup.handoff`（「在应用中导入」）：两者都是 API 契约（`README.md:96-100`）里的用户选择点。
...
| **2** | **只点真控件，不复制其逻辑** | 必须依次点击真实的 popup 控件「采集当前商品」（`popup.html:8`）与「在应用中导入」（`:9`），并点击真实的 `Confirm and submit`（`capture-receiver.tsx:145`，等按钮可点 `disabled={!scoped \|\| view.busy}`）；**不得**直接调 `capture1688(...)`、手拼 POST，也不得直发内部消息跳过 popup UI | 绕过应用侧上下文漂移保护与 `reserved` 置位，等于自建第二条提交路径 |
...
**两个用户选择点，一并记录。** ③ 契约里有**两处**要求用户自己做出选择，本次决定把两者都从「逐条」改为「批量启动一次 + 执行器代触发」，**两者都必须列入 §12 待同步**，不能只记一处：

| 契约位置 | 要求 | 本次变更 |
|---|---|---|
| `README.md:96-100` | *"The user sees a summary, then explicitly chooses to open the current app."*（即 popup 的「在应用中导入」控件 `popup.html:9`） | 由执行器代点；批量启动即为该选择 |
| `README.md:124-128` | *"Before any POST … obtain explicit confirmation"*（`capture-receiver.tsx:145`） | 同上 |
...
#### D2.2 本地队列必须**崩溃安全**（第八轮评审 #21）

阶段① 的那次本地写入是**唯一阻止重采的栅栏**。如果它只落在 OS cache 就宣告成功，用户提交后机器断电 ⇒ 重启后读回旧的 `captured` ⇒ 自动重采 + 二次发布。所以「写本地文件」不能是一句空话。

**协议要求（可执行）**：

1. **原子且落盘**：写临时文件 → 强制刷新到磁盘 → **原子替换**（POSIX `rename` / Windows `ReplaceFileW` 或 `MoveFileExW(MOVEFILE_REPLACE_EXISTING\|MOVEFILE_WRITE_THROUGH)`）→ 同步目录（POSIX）。整队列可用「单文件 + 原子替换」或「append-only + 每行校验和」。
2. **必须在发 `popup.handoff` 之前完成并确认**。写入失败或无法确认落盘 ⇒ **不得 handoff**（fail-closed）。⇒ 「写失败」的结果是**停下**，不是继续跑。
3. **不可读 / 损坏 / 截断 / 校验失败** ⇒ 受影响的条目一律按 **`outcome_unknown`** 处理，**绝不**回退为 `captured`/可重采。因为无法区分「已写但损坏」与「从未写过」。
4. **陈旧或顺序不可证**（例如文件存在、但缺少本轮该条的记录，或序号/时间无法证明顺序）⇒ 同上归 `outcome_unknown`。
5. **不做自动修复、不做自动回滚到旧版本**——那正是危险方向。不确定就停下交人。

**明确不做**：不为此引入数据库 / 消息队列 / Temporal。一个本地单文件队列用不上持久化基础设施（AGENTS：由实测瓶颈触发，不由假设预建）；也**不新增任何服务端状态**。
...
| **预写先于 handoff 且已落盘（§4 D2.2）** | 拦截本地写入/伪造写入失败 ⇒ 执行器**不发出** `popup.handoff`；可验证方式：写失败时无 `capture.read` 消息 |
| **损坏队列文件（§4 D2.2）** | 将队列文件截断 / 篡改校验和 / 删除当前条目 ⇒ 重启后受影响条目为 `outcome_unknown` 且**无新 POST**（**不**回退为 `captured`） |
| **掉电语义（人工检查项）** | 代码审查确认没有「写完就返回」的路径；写入路径经过 flush + 原子替换（S1 实现时以代码审查 + 一次人为 kill 验证，不做断电故障注入平台） |
...
4. ~~剩余额度怎么展示~~ **已定（协调方 2026-09-21，用户授权代办）：选 (a2)** —— **不新增响应字段**，仅在 429 `ACQUISITION_CAPACITY`（`product_acquisition_application.go:311`）时给出清晰提示。
   - 理由：终身上限 2048、单批 10 条 ⇒ 要 200+ 批才会耗尽；为它动契约（并在 web 侧同步 zod schema）与路线 B「零新增面」的前提不成比例。
   - **不选 (a1)，也不是否决它**：(a1) 转入 Backlog，由**真实撞到上限**或用户困惑触发（AGENTS：由实测触发，不由假设预建）。
   - **不选 (a3)**：本地计数不权威，会因已发布/失败行同样计入而给出错误印象。
5. ~~登录态探测~~ **已定：以「每条商品页导航」本身作为权威检查**（实际取不到就触发 §4 D1.3），整批开始前只做一次轻量确认（profile 能启动、能加载扩展）；**不额外发探测请求**（避免无谓的 1688 流量）。
6. ~~多执行器~~ **已定：不允许**。单 profile + 本地锁（§4 D3），同时只有一个写入执行器。
...
- **`extensions/1688-capture/README.md:96-100`（同类，必须与上一条一起处理）**：*"The user sees a summary, then **explicitly chooses to open the current app**."* 这是与上面**不同**的一处用户选择点（popup 的「在应用中导入」控件，`popup.html:9`）。第八轮评审 #20 指出我上一版**只列了 POST 确认、漏了这一处**。两者都因 §1.6 从「逐条」变为「批量启动一次」，故**必须一并列入待同步**；只改一处会让 README 仍与实际语义不符。
...
### 15.8 第八轮 2 条（针对 v3.4）

| # | 评审内容 | 分类 | 处置 |
|---|---|---|---|
| 20 | **handoff 也需要用户选择** | **BLOCKER（成立，已修；我漏了一处契约）** | 核实成立：`README.md:96-100` 另有一处要求——*"The user sees a summary, then **explicitly chooses to open the current app**."*，对应 popup 的「在应用中导入」控件（`popup.html:9`）。我上一版只处理了 POST 确认（`README.md:124-128`），**漏了 handoff 这一处**，导致 §12 的待同步清单不完整。**已修**：(a) §4 D1.2 新增「两个用户选择点」对照表，两处都归入「批量启动一次 + 执行器代触发」；(b) 护栏 2 从「只点真按钮」扩为「**只点真控件**」，显式包含 `popup.html:8`/`:9` 两个控件；(c) §12 新增 `README.md:96-100` 条目，并说明为何必须与 `:124-128` 一起改。同时**如实修正了 §4 D1.1 里一句过度声明**：原写「模拟点击…更贴合『这是用户操作』的语义」——这句站不住。扩展的消息准入（`background.ts:28`）只检查来源页面与 `tab`，**无法区分模拟与真实点击**；所以「用户自己选择」是**契约层要求，不是被代码强制的边界**，已改写并明确本设计**不**新增扩展侧校验 |
| 21 | **预写标记必须崩溃持久** | **BLOCKER（成立，已修）** | 核实成立且命中 AGENTS 的 BLOCKER 条件（重复且不可安全恢复的外部副作用）。本地文件是唯一重采栅栏；若只在 OS cache 确认就 handoff，提交后断电 ⇒ 重启读到旧 `captured` ⇒ 自动重采 + 二次发布。**已新增 §4 D2.2**：原子替换 + 强制落盘、**写未确认落盘则不 handoff**（fail-closed）、不可读/损坏/截断/校验失败一律 `outcome_unknown`（**不**回退为 `captured`）、陈旧或顺序不可证同理、**不做自动修复/回滚**；且明确不为此引入数据库/消息队列/Temporal。§10 新增三项可执行验收（含拦截写入 ⇒ 无 `capture.read`、损坏队列 ⇒ `outcome_unknown`） |

两条都成立。第 20 条是我**漏读契约**（只查了 POST 确认那一段），第 21 条是**把「写本地文件」当成已解决的问题**。加上 §15.6，共同教训仍然是「把语义正确误当成已实现/可实现」。
...
13. **补齐第二处用户选择点（`README.md:96-100` 的 handoff）**：第八轮评审 #20 指出我漏了这一处，只记了 POST 确认。§4 D1.2 新增两处对照表，护栏 2 扩为「只点真控件」（含 `popup.html:8`/`:9`），§12 新增对应条目；并修正 §4 D1.1 中「模拟点击更贴合用户操作语义」这句过度声明——扩展无法区分模拟与真实点击，该层属契约要求而非代码强制。
14. **新增 §4 D2.2（本地队列崩溃安全）**：第八轮评审 #21 指出本地文件是唯一重采栅栏，必须原子替换 + 强制落盘、写未落盘则不 handoff、不可读/损坏一律 `outcome_unknown`、不做自动修复/回滚。§10 新增三项验收。

## 17. 决策记录（2026-09-21，用户授权协调方代办）

用户就 §11 的余下问题回答「你来决定」。以下为**协调方决定**，逐条给出依据与可逆性。

| # | 决定 | 依据 | 可逆性 |
|---|---|---|---|
| 1 | **handoff（「在应用中导入」）也由执行器代点** | 直接衍生自 §1.6「只有遇验证码才需人处理」。它是与 POST 确认**不同**的一处选择点，但用户决定在语义上已覆盖 | 高——改回「人工点导入」只影响 §4 D1.2 一处 |
| 2 | **不改写 `extensions/1688-capture/README.md` 的契约文本**，记为**显式例外**并要求 owner 同步 | 它是已准入的 contract 文本，不属本 PR 范围；由非 owner 静默改写一个 consent 相关契约，比留一条可见的待同步项更危险 | 高——owner 可随时自行更新措辞 |
| 3 | **以上两处待同步不阻塞 S1**，但**阻塞任何面向用户的开放** | S1 只是本地内部驱动，没有真实用户；而「用户自己做出选择」这一层只有在真实使用时才成立 | — |
| 4 | **§11-4 选 (a2)**，不新增响应字段 | 2048 终身 / 单批 10 ⇒ 200+ 批才耗尽；动契约与「零新增面」不成比例 | 高——(a1) 留在 Backlog |
| 5 | **§11-5：以每条商品页导航为权威检查**，不额外发探测请求 | 真实取不到就触发 §4 D1.3；额外探测只会增加 1688 流量（风控角度不利） | — |
| 6 | **§11-6：不允许并行多执行器** | 单 profile 只能被一个浏览器占用；由本地锁保证 | 中——放开需先解决 profile 并发 |
| 7 | **批准 S1 开工，但限定实现范围** | 设计已收敛到可实现；继续纯文档对轮不再产生新信息 | 高 |
| 8 | **S1 的验证只用 fixture / 假扩展**；**真实 1688 账号实测不自行执行** | 用户此前明确：已登录账号已带自动化历史，不建议再对它做自动化访问；且建立登录态 profile 必须由用户本人操作 | — |

**第 8 条是本次唯一不代用户决定的事**，原因不是流程洁癖而是风控与账号安全：真实链路验证必须用那个已登录的 1688 账号，而重复自动化访问本身可能损坏账号。因此 S1 交付的是**代码 + 基于 fixture 的证据**；「在真实账号上跑通一条」作为**用户授权的独立验收**保留，本文档保留 `NOT_RUN`，不用 fixture 结果冒充。

### 17.1 S1 的边界（据第 7/8 条）

- **做**：用 `github.com/mxschmitt/playwright-go` 启动 fingerprint-chromium → 加载 `extensions/1688-capture/dist` → 驱动 popup 控件（`popup.html:8`/`:9`）→ 打开采集页 → 驱动 `Confirm and submit` → 回读终态；含 §4 D2.2 的崩溃安全队列读写。
- **不做**：不加服务端路由、不改 ③、不改采集契约、不新增 producer、不做定时/多账号/代理。
- **证据来源**：fixture 页面 + 假扩展（覆盖 §10 的确定性项：单条等价性、scope 固定、reset、预写落盘、损坏队列）。**§10 中依赖真实 1688 风控/登录态的项保留 `NOT_RUN`**。
- **切片**：仍按 §14 的 S1→S5，不因第 7 条而跳过 S3/S5。
