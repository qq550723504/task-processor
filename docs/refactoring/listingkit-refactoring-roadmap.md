...
| 对象 | 状态 | 证据 |
| --- | --- | --- |
| Go Listing Control Plane 代码路径 | leader rollout validated, production hardening open | `cmd/listing-control-plane`、`internal/listingcontrol`、`internal/app/runtime/listingcontrol`、`scripts/build-push-deploy-listing-control-plane.ps1` 和 `deployments/kubernetes/shein-listing/overlays/prod-auto-shard-statefulset/listing-control-plane.yaml` 已落库；Redis leader lease 已接入 control-plane cycle 和 status 输出，`docs/product/validation/runs/2026-06-24-listing-control-plane-leader-rollout.md` 已验证双实例 rollout 下只有 leader 执行 recovery/dispatch，standby 保持 ready 且不调度。dispatch skip/delay reason 已在代码层持久化到 import task 的 `stage/reason_code/error_message/remark`，并追加 `listing_dispatch_event` 审计事实；daily limit capacity 已接入 Store Runtime。当前缺口集中在 leader 主动删除接管、任务列表运营验证和真实 rollback 验证。 |
