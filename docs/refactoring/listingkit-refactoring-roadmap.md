...
| SHEIN SDS image matching helpers | migrated | SDS mockup 到 ImageSet 转换、SKU/color key 归一、source_sds_sku/SupplierSKU 匹配和 image set merge 迁入 `internal/publishing/shein/sds_images.go`；ListingKit SDS 图片流程直接调用 publishing helpers，仅保留前端请求 DTO 适配与编排逻辑。 |
