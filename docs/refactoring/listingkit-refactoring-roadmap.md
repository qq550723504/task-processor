...
| SHEIN revision patch application | migrated | editor revision 的 category/attribute/sale attribute/SKC/SKU patch 应用规则迁入 `internal/marketplace/shein/workspace/revision_apply_patch.go`；ListingKit revision apply support 仅保留生产调用的兼容 wrapper 和整体 apply 编排，已删除未调用的 SKU patch 直通 wrapper。 |
...
| SHEIN repair revision seed construction | migrated | repair patch payload、repair reason、revision input 和最小 editor skeleton seed 构造迁入 `internal/marketplace/shein/workspace/repair_revision.go`；ListingKit 仅保留生产使用的 bundle/artifacts 入口来包装本包 `ApplyRevisionRequest` 和 validation preview，已删除未调用的 repair revision 小 wrapper。 |
