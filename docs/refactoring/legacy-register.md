...
- `TestCurrentOwnersDoNotImportLegacyListingKitRoot` and `current_owners_legacy_listingkit_root` block the mixed root service from Product, Listing, Marketplace, Agent, CommerceTool, Console and BusinessTask packages, including tests. Existing extracted leaf contracts (for example `internal/listingkit/core`) remain subject to #29 and their existing boundary rules.
