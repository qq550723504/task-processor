...
| `internal/app/httpapi.newImageAgentAuthorizedAssetCatalog(listingkitstore.NewTaskRepository(...))` and `imageAgentCatalogFromTask*` accepting root `listingkit.Task` | RETIRE input bridge | ImageAgent domain/workflow/store, Temporal worker, `cmd/image-agent-temporal-worker` and deployment definitions remain **CURRENT**. `NewImageAgentOrganizationApplication` is current review governance but isolated and **NOT_PRESENT** in default composition. | Deterministic asset selection/validation only when moved behind a current Product/Asset-owned catalog input; ImageAgent behavior itself is not legacy. | Replace the default task catalog with an admitted authoritative current input and switch callers, then delete the Task bridge. No fallback/dual catalog and no blanket retirement of ImageAgent. |
