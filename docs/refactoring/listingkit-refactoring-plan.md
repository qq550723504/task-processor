...
- generation navigation target identity support is now split by helper family, so root `listingkit` keeps target-identity entrypoints plus clone-shape homes while descriptor/resource/cache/dispatch builder helpers live in a dedicated support file.
- `internal/listingkit/sheinsync/model.go` is now split by model family, so the SHEIN sync domain no longer mixes synced-product state, sync-job state, candidate review state, and enrollment run/item state in one oversized model file.
- `internal/listingkit/httpapi/builders_repositories.go` is now split by repository-builder family, so the HTTP runtime wiring layer no longer mixes ListingKit, admin, and shared support repository fallback selection in one central builder file.
- `internal/listingkit/httpapi/zitadel_auth.go` is now split by auth support family, so the HTTP auth shell no longer mixes runtime config state, token verification middleware, and route-level authorization policy in one oversized ZITADEL integration file.
- `internal/listingkit/httpapi/builders.go` is now split by support family, so HTTP runtime setup no longer mixes repository schema bootstrap, public repository fallback builders, DB-backed repository factories, image-upload store construction, legacy tenant bridge probing, and recovery sweep config parsing in one framework file.
- `internal/listingkit/api/shein_sync_handler.go` is now split by SHEIN sync handler family, so the API shell no longer mixes synced-product actions, candidate review flows, enrollment execution, and shared tenant/store parsing helpers in one oversized request handler file.
- `internal/listingkit/api/shein_sync_summary_handler.go` is now split by summary handler family, so the API shell no longer mixes dashboard endpoints, enrollment-run listing, and store-summary aggregation helpers in one oversized SHEIN summary file.
- `internal/listingkit/api/studio_async_jobs_handler.go` is now split by async-job support family, so the API shell no longer mixes async request DTOs, job-store persistence adapters, HTTP entrypoints, background execution flow, and logging/session sync helpers in one oversized studio async handler file.
- `internal/listingkit/httpapi/routes.go` is now split by route-contract family, so the HTTP shell no longer keeps task, settings, store/subscription, admin, SHEIN sync, and top-level composite handler surfaces in one oversized interface catalog file.
- `internal/listingkit/httpapi/routes_descriptors.go` is now also split by route-registration family, so HTTP route wiring no longer keeps root entrypoints, settings/store-subscription, admin, task/studio, and SHEIN sync descriptor assembly in one oversized registration file.
- `internal/listingkit/httpapi/bootstrap.go` is now reduced to thin service/module entrypoints, while contracts, validation, closer helpers, and module-service environment/wiring move into dedicated bootstrap support families.
- `internal/listingkit/httpapi/bootstrap_repositories.go` is now split by repository family, so HTTP service assembly no longer keeps repository state structs, core/late-core builders, admin builders, and merge/assembly glue in one oversized bootstrap file.
- `internal/listingkit/httpapi/builders_db_repositories.go` is now split by DB factory family, with shared DB/bootstrap support separated from ListingKit, admin, and support-store repository factories, so HTTP persistence factories no longer sit in one oversized constructor file.
- `internal/listingkit/store/task_repo.go` is now split by repository family, so the Gorm task repository no longer keeps list filtering, retry/state transitions, cache persistence, and access-scope helpers in one oversized store file.
- generation review patch support is now split by helper family, so root `listingkit` keeps review-session patch assembly while diff/summary/cards/previews/equality helpers live in a dedicated support file.
- generation navigation model support is now split by helper family, so root `listingkit` keeps target/descriptor/request-response model definitions while dispatch execution, panel update, and recovery descriptor models live in a dedicated support file.
- request model support is now split by helper family, so root `listingkit` keeps core task request definitions while studio media, SDS option, and submit/settings/search request DTOs live in a dedicated support file.
- studio session model support is now split by helper family, so root `listingkit` keeps session status plus persisted session/design entities while selection state, batch request/list, and gallery-facing DTOs live in a dedicated support file.
- workflow studio SDS metadata support is now split by helper family, so root `listingkit` keeps category/attribute/specification/variant/selling-point builders while style-dimension, SKU normalization, and shared string helper utilities live in a dedicated support file.
- task studio session service support is now split by helper family, so root `listingkit` keeps constructor plus public session entrypoints while update-shape predicates, detail/log helpers, stale-write guards, and runner ensure wiring live in a dedicated support file.
- generation recovery summary support is now split by helper family, so root `listingkit` keeps recovery-summary assembly and response/overview injection entrypoints while descriptor cloning, recovery-target/profile arbitration, and CTA preference helpers live in a dedicated support file.
- SDS SHEIN image support is now split by helper family, so root `listingkit` keeps selected/template/variant image application entrypoints while SKC/SKU lookup, image-set construction/merge, and color-key normalization helpers live in a dedicated support file.
- SHEIN final draft support is now split by helper family, so root `listingkit` keeps final-draft application plus draft/preview SKC synchronization entrypoints while image ordering, fallback selection, SKC lookup, preview image-info conversion, and role-override normalization helpers live in a dedicated support file.
- studio batch mem repository support is now split by helper family, so root `listingkit` keeps graph creation/read flows while generation-graph replacement, review replacement, mutable record updates, and detail-graph assembly live in a dedicated support file.
- model result support is now split by helper family, so root `listingkit` keeps core execution/result snapshot models while recovery-action summary DTOs and platform publish package models live in a dedicated support file.
- generation overview quality support is now split by helper family, so root `listingkit` keeps overview decisioning plus top-level filter routing while quality-grade, blocking-platform, and preview-readiness helpers live in a dedicated support file.
...
- task studio batch service runner support is now split by helper family, so root `listingkit` keeps public batch generation/review/retry/task entrypoints while runner ensure wiring and session/batch updater extraction live in a dedicated support file.
- SDS baseline readiness support is now split by helper family, so root `listingkit` keeps baseline cache/readiness entrypoints while login-status reconcile, validation-status normalization, backward-compatibility revalidate, and overall-status derivation helpers live in a dedicated support file.
- task submission recovery route support is now split by helper family, so root `listingkit` keeps recovery service construction plus top-level acquire/recover entrypoints while lease-preview helpers, local-vs-remote route predicates, and workflow-start-failure persistence helpers live in a dedicated support file.
- SHEIN submit image upload support is now split by helper family, so root `listingkit` keeps submit-image entrypoints, counters, and cache classifiers while image-ref collection, concurrent upload execution, and color-block upload fallback helpers live in a dedicated support file.
- SHEIN repair clone support is now split by helper family, so root `listingkit` keeps repair revision/artifact assembly while patch-clone helpers, validation-preview cloning, and revision-preview deep-copy helpers live in a dedicated support file.
- SHEIN repair revision support is now split by helper family, so root `listingkit` keeps only repair shell types while revision-bundle assembly, request/input shaping, artifact assembly, and validation-preview building live in dedicated support files.
- SHEIN submit SKU style support is now split by helper family, so root `listingkit` keeps pricing rename/reconcile helpers while style-suffix derivation, discriminator shaping, and submit-token classifiers live in a dedicated support file.
- task submission state persistence support is now split by helper family, so root `listingkit` keeps submit-state construction plus direct phase/response persistence entrypoints while result-runner finish logic, failure persistence, and success/failure fallback helpers live in a dedicated support file.
...
- workflow SDS sync support is now split by helper family, so root `listingkit` keeps the three SDS sync orchestration entrypoints while stage setup, failure/success finalization, and variant failure summary helpers live in a dedicated support file.
...
- submit wiring home construction now retains only the root submit assembly, support bundles, and collaborator ensure seams, while submission dependency resolution helpers plus execution/state/recovery config builders live in a dedicated support file, so root `listingkit` no longer mixes assembly wiring with low-level resolver/config helper families in one broad submit-wiring file.
...
- studio session adapter home construction now retains only the root runner constructors, while repository adapters, session record/status mutation helpers, and general metadata patch logic live in a dedicated support file, so root `listingkit` no longer mixes constructor seams with session-state helper families in one broad studio adapter file.
...
- `internal/listingkit/sheinsync/service.go` is now split by service family, so the SHEIN sync home no longer mixes service shell wiring, sync-job orchestration, supplemental snapshot loading, and synced-record/time helper families in one oversized service file.
- `internal/listingkit/sheinsync/enrollment_service.go` is now split by service family, so the SHEIN enrollment home no longer mixes service shell wiring, candidate-query selection, run execution flow, and outcome-persistence/status helper families in one oversized service file.
- `internal/listingkit/store/shein_sync_repo.go` is now split by repository family, so the SHEIN sync Gorm repository no longer mixes synced-product persistence, sync-job history, enrollment candidate/run/item persistence, and shared pagination/filter helpers in one oversized store file.
- `internal/listingkit/store/shein_sync_mem_store.go` is now split by repository family, so the SHEIN sync in-memory repository no longer mixes synced-product persistence, sync-job history, enrollment candidate/run/item persistence, and shared query/clone helpers in one oversized store file.
- `internal/listingkit/api/handler.go` and adjacent subscription shell files are now split by handler family, so the ListingKit API shell no longer mixes handler state/contracts, option-wiring/construction, task endpoints, platform subscription endpoints, and subscription guard helpers in one broad handler layer.
- `internal/listingkit/httpapi/routes_descriptor_admin.go` is now split by route-descriptor family, so the admin HTTP wiring layer no longer mixes store/import-task, rule/strategy, generation-topic, and catalog-data route registration in one oversized descriptor file.
