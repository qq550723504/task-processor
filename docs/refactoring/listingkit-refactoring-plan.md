...
- submit collaborator config assembly now also lives directly beside shared root-side submitter/support/assembly/temporal wiring bundles within each ensure path, so root `listingkit` no longer rebuilds the same dependency snapshots or keep a separate wrapper-only submit wiring file across recovery/requeue, submission core, managed submission, and Temporal collaborator constructors.
