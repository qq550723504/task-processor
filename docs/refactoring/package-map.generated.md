# Package Map

Generated by scripts/dependency-baseline.ps1.

## Packages

- `task-processor/cmd/product-listing-api`
- `task-processor/cmd/shein-listing`
- `task-processor/cmd/temu-listing`
- `task-processor/internal/amazon`
- `task-processor/internal/amazon/api`
- `task-processor/internal/amazon/attribute`
- `task-processor/internal/amazon/image`
- `task-processor/internal/amazon/listing`
- `task-processor/internal/amazon/llm`
- `task-processor/internal/amazon/model`
- `task-processor/internal/amazon/pipeline`
- `task-processor/internal/amazon/schema`
- `task-processor/internal/amazonlisting`
- `task-processor/internal/amazonlisting/api`
- `task-processor/internal/amazonlisting/httpapi`
- `task-processor/internal/amazonlisting/store`
- `task-processor/internal/app/bootstrap`
- `task-processor/internal/app/bootstrap/fetchers`
- `task-processor/internal/app/bootstrap/processors`
- `task-processor/internal/app/bootstrap/resources`
- `task-processor/internal/app/bootstrap/schedulers`
- `task-processor/internal/app/consumer`
- `task-processor/internal/app/crawler/distributed`
- `task-processor/internal/app/httpapi`
- `task-processor/internal/app/ports`
- `task-processor/internal/app/runner`
- `task-processor/internal/app/runtime`
- `task-processor/internal/app/runtime/listing`
- `task-processor/internal/app/scheduler`
- `task-processor/internal/app/task`
- `task-processor/internal/app/taskstatus`
- `task-processor/internal/app/updater`
- `task-processor/internal/app/worker`
- `task-processor/internal/asset`
- `task-processor/internal/asset/bundle`
- `task-processor/internal/asset/generation`
- `task-processor/internal/asset/recipe`
- `task-processor/internal/asset/repository`
- `task-processor/internal/authz`
- `task-processor/internal/catalog`
- `task-processor/internal/catalog/canonical`
- `task-processor/internal/compatibility/listingkit`
- `task-processor/internal/core/config`
- `task-processor/internal/core/errors`
- `task-processor/internal/core/lifecycle`
- `task-processor/internal/core/logger`
- `task-processor/internal/core/metrics`
- `task-processor/internal/crawler/alibaba1688`
- `task-processor/internal/crawler/alibaba1688/extractor`
- `task-processor/internal/crawler/alibaba1688/model`
- `task-processor/internal/crawler/amazon`
- `task-processor/internal/crawler/amazon/browser`
- `task-processor/internal/crawler/amazon/extractor`
- `task-processor/internal/crawler/amazon/variations`
- `task-processor/internal/crawler/fetcher`
- `task-processor/internal/crawler/shared`
- `task-processor/internal/crawler/shared/browser`
- `task-processor/internal/domain/task`
- `task-processor/internal/httpbootstrap`
- `task-processor/internal/httproute`
- `task-processor/internal/imageasset`
- `task-processor/internal/infra/auth`
- `task-processor/internal/infra/clients/management`
- `task-processor/internal/infra/clients/management/api`
- `task-processor/internal/infra/clients/grsai`
- `task-processor/internal/infra/clients/openai`
- `task-processor/internal/infra/database`
- `task-processor/internal/infra/httpx`
- `task-processor/internal/infra/lock`
- `task-processor/internal/infra/metrics`
- `task-processor/internal/infra/monitoring`
- `task-processor/internal/infra/rabbitmq`
- `task-processor/internal/infra/redisclient`
- `task-processor/internal/infra/resilience`
- `task-processor/internal/infra/storage`
- `task-processor/internal/infra/worker`
- `task-processor/internal/integration/crawler`
- `task-processor/internal/integration/crawler/a1688`
- `task-processor/internal/integration/crawler/amazon`
- `task-processor/internal/kernel/module`
- `task-processor/internal/listing/preview`
- `task-processor/internal/listing/studio`
- `task-processor/internal/listing/submission`
- `task-processor/internal/listingadmin`
- `task-processor/internal/listingkit`
- `task-processor/internal/listingkit/api`
- `task-processor/internal/listingkit/core`
- `task-processor/internal/listingkit/generation`
- `task-processor/internal/listingkit/httpapi`
- `task-processor/internal/listingkit/reviewstore`
- `task-processor/internal/listingkit/service`
- `task-processor/internal/listingkit/service/studio`
- `task-processor/internal/listingkit/sheinsync`
- `task-processor/internal/listingkit/store`
- `task-processor/internal/listingkit/studiostore`
- `task-processor/internal/listingkit/temporal`
- `task-processor/internal/listingkit/workflow`
- `task-processor/internal/listingkit/workspace`
- `task-processor/internal/listingkit/workspace/shein`
- `task-processor/internal/listingsubscription`
- `task-processor/internal/marketplace/amazon/api`
- `task-processor/internal/marketplace/amazon/model`
- `task-processor/internal/marketplace/amazon/publishing`
- `task-processor/internal/marketplace/amazon/workspace`
- `task-processor/internal/marketplace/shein/publishing`
- `task-processor/internal/marketplace/shein/workspace`
- `task-processor/internal/model`
- `task-processor/internal/pipeline`
- `task-processor/internal/pipeline/handlers`
- `task-processor/internal/pkg/appenv`
- `task-processor/internal/pkg/cache`
- `task-processor/internal/pkg/downloader`
- `task-processor/internal/pkg/fileio`
- `task-processor/internal/pkg/goroutine`
- `task-processor/internal/pkg/hashx`
- `task-processor/internal/pkg/httpapicmd`
- `task-processor/internal/pkg/httpclient`
- `task-processor/internal/pkg/imagex`
- `task-processor/internal/pkg/jsonx`
- `task-processor/internal/pkg/mathx`
- `task-processor/internal/pkg/perf`
- `task-processor/internal/pkg/ptr`
- `task-processor/internal/pkg/recovery`
- `task-processor/internal/pkg/resilience`
- `task-processor/internal/pkg/skugen`
- `task-processor/internal/pkg/strx`
- `task-processor/internal/pkg/timeout`
- `task-processor/internal/pkg/timex`
- `task-processor/internal/pkg/types`
- `task-processor/internal/pkg/watermark`
- `task-processor/internal/platformbase`
- `task-processor/internal/platforms`
- `task-processor/internal/platforms/amazon`
- `task-processor/internal/platforms/shein`
- `task-processor/internal/platforms/temu`
- `task-processor/internal/platformtask`
- `task-processor/internal/ports`
- `task-processor/internal/pricing`
- `task-processor/internal/processor`
- `task-processor/internal/product`
- `task-processor/internal/product/sourcing`
- `task-processor/internal/productenrich`
- `task-processor/internal/productenrich/api`
- `task-processor/internal/productenrich/enrich`
- `task-processor/internal/productenrich/httpapi`
- `task-processor/internal/productenrich/pipeline`
- `task-processor/internal/productenrich/store`
- `task-processor/internal/productimage`
- `task-processor/internal/productimage/domain`
- `task-processor/internal/productimage/httpapi`
- `task-processor/internal/productimage/pipeline`
- `task-processor/internal/productimage/providers`
- `task-processor/internal/productimage/store`
- `task-processor/internal/prompt`
- `task-processor/internal/promptmgmt`
- `task-processor/internal/promptmgmt/api`
- `task-processor/internal/publishing/common`
- `task-processor/internal/publishing/shein`
- `task-processor/internal/publishing/sheinmanaged`
- `task-processor/internal/scheduler`
- `task-processor/internal/sds/adapter`
- `task-processor/internal/sds/client`
- `task-processor/internal/sds/design`
- `task-processor/internal/sds/httpapi`
- `task-processor/internal/sds/httpbootstrap`
- `task-processor/internal/sds/template`
- `task-processor/internal/sds/usecase`
- `task-processor/internal/sds/workflow`
- `task-processor/internal/sdslogin`
- `task-processor/internal/sdslogin/bootstrap`
- `task-processor/internal/shared/tenantctx`
- `task-processor/internal/shein`
- `task-processor/internal/shein/activity`
- `task-processor/internal/shein/addresscopy`
- `task-processor/internal/shein/aicache`
- `task-processor/internal/shein/api`
- `task-processor/internal/shein/api/attribute`
- `task-processor/internal/shein/api/category`
- `task-processor/internal/shein/api/image`
- `task-processor/internal/shein/api/marketing`
- `task-processor/internal/shein/api/other`
- `task-processor/internal/shein/api/pricing`
- `task-processor/internal/shein/api/product`
- `task-processor/internal/shein/api/translate`
- `task-processor/internal/shein/api/warehouse`
- `task-processor/internal/shein/authorizedbrand`
- `task-processor/internal/shein/category`
- `task-processor/internal/shein/client`
- `task-processor/internal/shein/content`
- `task-processor/internal/shein/context`
- `task-processor/internal/shein/generationtopics`
- `task-processor/internal/shein/inventory`
- `task-processor/internal/shein/managedclient`
- `task-processor/internal/shein/mapping`
- `task-processor/internal/shein/pipeline`
- `task-processor/internal/shein/pricing`
- `task-processor/internal/shein/product`
- `task-processor/internal/shein/product/attribute`
- `task-processor/internal/shein/product/attribute/sale`
- `task-processor/internal/shein/product/build`
- `task-processor/internal/shein/product/image`
- `task-processor/internal/shein/product/skc`
- `task-processor/internal/shein/product/sku`
- `task-processor/internal/shein/product/variant`
- `task-processor/internal/shein/productdata`
- `task-processor/internal/shein/productsync`
- `task-processor/internal/shein/publish`
- `task-processor/internal/shein/scheduler`
- `task-processor/internal/shein/sherr`
- `task-processor/internal/shein/store`
- `task-processor/internal/shein/submitprep`
- `task-processor/internal/shein/translate`
- `task-processor/internal/shein/validation`
- `task-processor/internal/sheinbridge/saleattribute`
- `task-processor/internal/sheinlogin`
- `task-processor/internal/sheinlogin/bootstrap`
- `task-processor/internal/sheinloginmanaged`
- `task-processor/internal/state`
- `task-processor/internal/taskrpcapi`
- `task-processor/internal/taskstatus`
- `task-processor/internal/temu`
- `task-processor/internal/temu/ai`
- `task-processor/internal/temu/api`
- `task-processor/internal/temu/api/category`
- `task-processor/internal/temu/api/client`
- `task-processor/internal/temu/api/image`
- `task-processor/internal/temu/api/inventory`
- `task-processor/internal/temu/api/pricing`
- `task-processor/internal/temu/api/product`
- `task-processor/internal/temu/api/query`
- `task-processor/internal/temu/api/template`
- `task-processor/internal/temu/api/types`
- `task-processor/internal/temu/bulkrelist`
- `task-processor/internal/temu/category`
- `task-processor/internal/temu/context`
- `task-processor/internal/temu/filter`
- `task-processor/internal/temu/format`
- `task-processor/internal/temu/handlerbase`
- `task-processor/internal/temu/image`
- `task-processor/internal/temu/pricing`
- `task-processor/internal/temu/product`
- `task-processor/internal/temu/property`
- `task-processor/internal/temu/rules`
- `task-processor/internal/temu/scheduler`
- `task-processor/internal/temu/sku`
- `task-processor/internal/temu/spec`
- `task-processor/internal/temu/store`
- `task-processor/internal/temu/sync`
- `task-processor/internal/temu/template`
- `task-processor/internal/tenantbridge`
- `task-processor/internal/workspace/shein`
- `task-processor/internal/zitadelprovision`
- `task-processor/tests`
